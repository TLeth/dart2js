...
class ResolutionWorkItem extends WorkItem {
  ...
  ResolutionWorkItem(AstElement element, ItemCompilationContext compilationContext)
...
class CodegenRegistry extends Registry {
  ...
  void registerTypeVariableBoundsSubtypeCheck(DartType subtype, DartType supertype) {
...
class CodegenWorkItem extends WorkItem {
  ...
  CodegenWorkItem(AstElement element, ItemCompilationContext compilationContext)
      : this.resolutionTree = element.resolvedAst.elements,
        super(element, compilationContext) {
    assert(invariant(element, resolutionTree != null, message: 'Resolution tree is null for $element in codegen work item'));
  }
...
abstract class Backend {
  ...
  void registerIsCheckForCodegen(DartType type, Enqueuer enqueuer, Registry registry) {}
  ...
  bool isBackendLibrary(LibraryElement library) {
    ...
    if (canonicalUri == js_backend.JavaScriptBackend.DART_JS_HELPER || canonicalUri == js_backend.JavaScriptBackend.DART_INTERCEPTORS) {
      return true;
    }
    return false;
  }
...
abstract class Compiler implements DiagnosticListener {
  ...
  static final Uri DART_NATIVE_TYPED_DATA = new Uri(scheme: 'dart', path: '_native_typed_data');
  ...
  final Registry mirrorDependencies = new ResolutionRegistry.internal(null, new TreeElementMapping(null));
  ...
  static const String UNDETERMINED_BUILD_ID = "build number could not be determined";

  final Selector iteratorSelector = new Selector.getter('iterator', null);
  final Selector currentSelector = new Selector.getter('current', null);
  final Selector moveNextSelector = new Selector.call('moveNext', null, 0);
  final Selector noSuchMethodSelector = new Selector.call(Compiler.NO_SUCH_METHOD, null, Compiler.NO_SUCH_METHOD_ARG_COUNT);
  final Selector symbolValidatedConstructorSelector = new Selector.call('validated', null, 1);
  final Selector fromEnvironmentSelector = new Selector.callConstructor('fromEnvironment', null, 2);
  ...
  Compiler({this.enableTypeAssertions: false, this.enableUserAssertions: false, this.trustTypeAnnotations: false, this.enableConcreteTypeInference: false, bool disableTypeInferenceFlag: false, this.maxConcreteTypeSize: 5, this.enableMinification: false, this.enableNativeLiveTypeAnalysis: false, bool emitJavaScript: true, bool dart2dartMultiFile: false, bool generateSourceMap: true, bool analyzeAllFlag: false, bool analyzeOnly: false, this.analyzeMain: false, bool analyzeSignaturesOnly: false, this.preserveComments: false, this.verbose: false, this.sourceMapUri: null, this.outputUri: null, this.buildId: UNDETERMINED_BUILD_ID, this.terseDiagnostics: false, this.dumpInfo: false, this.showPackageWarnings: false, this.useContentSecurityPolicy: false, this.suppressWarnings: false, bool hasIncrementalSupport: false, api.CompilerOutputProvider outputProvider, List<String> strips: const []})
      : this.disableTypeInferenceFlag = disableTypeInferenceFlag || !emitJavaScript,
        this.analyzeOnly = analyzeOnly || analyzeSignaturesOnly || analyzeAllFlag,
        ...
        this.outputProvider = (outputProvider == null) ? NullSink.outputProvider : outputProvider {
    ...
    globalDependencies = new CodegenRegistry(this, new TreeElementMapping(null));

    closureMapping.ClosureNamer closureNamer;
    if (emitJavaScript) {
      js_backend.JavaScriptBackend jsBackend = new js_backend.JavaScriptBackend(this, generateSourceMap);
      closureNamer = jsBackend.namer;
      backend = jsBackend;
    } else {
      closureNamer = new closureMapping.ClosureNamer();
      backend = new dart_backend.DartBackend(this, strips, multiFile: dart2dartMultiFile);
    }

    tasks = [libraryLoader = new LibraryLoaderTask(this), scanner = new ScannerTask(this), dietParser = new DietParserTask(this), parser = new ParserTask(this), patchParser = new PatchParserTask(this), resolver = new ResolverTask(this, backend.constantCompilerTask), closureToClassMapper = new closureMapping.ClosureTask(this, closureNamer), checker = new TypeCheckerTask(this), irBuilder = new IrBuilderTask(this), typesTask = new ti.TypesTask(this), constants = backend.constantCompilerTask, deferredLoadTask = new DeferredLoadTask(this), mirrorUsageAnalyzerTask = new MirrorUsageAnalyzerTask(this), enqueuer = new EnqueueTask(this), dumpInfoTask = new DumpInfoTask(this), reuseLibraryTask = new GenericTask('Reuse library', this),];
  ...
  void internalError(Spannable node, reason) {
    ...
    reportDiagnosticInternal(node, MessageKind.GENERIC, {
      'text': message
  ...
  void unhandledExceptionOnElement(Element element) {
    ...
    hasCrashed = true;
    reportDiagnostic(element, MessageKind.COMPILER_CRASHED.message(), api.Diagnostic.CRASH);
    pleaseReportCrash();
  }

  void pleaseReportCrash() {
    print(MessageKind.PLEASE_REPORT_THE_CRASH.message({
      'buildId': buildId
    }));
  }
  ...
  Element _elementFromHInstruction(HInstruction instruction) {
    return instruction.sourceElement is Element ? instruction.sourceElement : null;
  }
  ...
  void log(message) {
    reportDiagnostic(null, MessageKind.GENERIC.message({
      'text': '$message'
      ...
      try {
        if (!hasCrashed) {
          ...
          if (error is SpannableAssertionFailure) {
            ...
            reportDiagnostic(new SourceSpan(uri, 0, 0), MessageKind.COMPILER_CRASHED.message(), api.Diagnostic.CRASH);
          }
          pleaseReportCrash();
        }
      } catch (doubleFault) {
  ...
  Future onLibrariesLoaded(Map<Uri, LibraryElement> loadedLibraries) {
    return new Future.sync(() {
      ...
      proxyConstant = resolver.constantCompiler.compileConstant(coreLibrary.find('proxy')).value;

      // TODO(johnniwinther): Move this to the JavaScript backend.
      LibraryElement jsHelperLibrary = loadedLibraries[js_backend.JavaScriptBackend.DART_JS_HELPER];
      if (jsHelperLibrary != null) {
        patchConstant = resolver.constantCompiler.compileConstant(jsHelperLibrary.find('patch')).value;
      }

      if (preserveComments) {
        return libraryLoader.loadLibrary(DART_MIRRORS).then((LibraryElement libraryElement) {
  ...
  Element findRequiredElement(LibraryElement library, String name) {
    ...
    if (element == null) {
      internalError(library, "The library '${library.canonicalUri}' does not contain required " "element: '$name'.");
    }
    return element;
  }
  ...
  void onClassResolved(ClassElement cls) {
    if (mirrorSystemClass == cls) {
      ...
      symbolValidatedConstructor = symbolImplementationClass.lookupConstructor(symbolValidatedConstructorSelector);
      ...
      stringEnvironment = stringClass.lookupConstructor(fromEnvironmentSelector);
  ...
  void initializeCoreClasses() {
    ...
    if (!missingCoreClasses.isEmpty) {
      internalError(coreLibrary, 'dart:core library does not contain required classes: ' '$missingCoreClasses');
    }
  }
  ...
  Element get unnamedListConstructor {
    ...
    Selector callConstructor = new Selector.callConstructor("", listClass.library);
    return _unnamedListConstructor = listClass.lookupConstructor(callConstructor);
  }
  ...
  Element get filledListConstructor {
    ...
    Selector callConstructor = new Selector.callConstructor("filled", listClass.library);
    return _filledListConstructor = listClass.lookupConstructor(callConstructor);
  }
  ...
  void computeMain() {
    ...
    if (main == null) {
      ...
      if (parameters.requiredParameterCount > 2) {
        ...
        parameters.orderedForEachParameter((Element parameter) {
          if (index++ < 2) return;
          errorElement = new ErroneousElementX(MessageKind.MAIN_WITH_EXTRA_PARAMETER, {
            'main': MAIN
          }, MAIN, parameter);
    ...
    if (errorElement != null && errorElement.isSynthesized) {
      reportWarning(errorElement, errorElement.messageKind, errorElement.messageArguments);
    }
  }
  ...
  void processQueue(Enqueuer world, Element main) {
    ...
    if (main != null && !main.isErroneous) {
      ...
      if (mainMethod.computeSignature(this).parameterCount != 0) {
        ...
        world.registerInstantiatedClass(backend.listImplementation, globalDependencies);
        world.registerInstantiatedClass(backend.stringImplementation, globalDependencies);
  ...
  checkQueues() {
    ...
    for (Element e in new Set.from(resolved)) {
      if (e.isClass || e.isField || e.isTypeVariable || e.isTypedef || identical(e.kind, ElementKind.ABSTRACT_FIELD)) {
        resolved.remove(e);
      }
  ...
  void analyzeElement(Element element) {
    assert(invariant(element, element.impliesType || element.isField || element.isFunction || element.isGenerativeConstructor || element.isGetter || element.isSetter, message: 'Unexpected element kind: ${element.kind}'));
    assert(invariant(element, element is AnalyzableElement, message: 'Element $element is not analyzable.'));
    assert(invariant(element, element.isDeclaration));
  ...
  void analyze(ResolutionWorkItem work, ResolutionEnqueuer world) {
    ...
    assert(invariant(work.element, !work.isAnalyzed(), message: 'Element ${work.element} has already been analyzed'));
  ...
  void reportError(Spannable node, MessageKind messageKind, [Map arguments = const {}]) {
    reportDiagnosticInternal(node, messageKind, arguments, api.Diagnostic.ERROR);
  }
   ...
   */
  void reportFatalError(Spannable node, MessageKind messageKind, [Map arguments = const {}]) {
    reportError(node, messageKind, arguments);
    // TODO(ahe): Make this only abort the current method.
    throw new CompilerCancelledException('Error: Cannot continue due to previous error.');
  }

  void reportWarning(Spannable node, MessageKind messageKind, [Map arguments = const {}]) {
    reportDiagnosticInternal(node, messageKind, arguments, api.Diagnostic.WARNING);
  }

  void reportInfo(Spannable node, MessageKind messageKind, [Map arguments = const {}]) {
    reportDiagnosticInternal(node, messageKind, arguments, api.Diagnostic.INFO);
  }

  void reportHint(Spannable node, MessageKind messageKind, [Map arguments = const {}]) {
    reportDiagnosticInternal(node, messageKind, arguments, api.Diagnostic.HINT);
  }

  void reportDiagnosticInternal(Spannable node, MessageKind messageKind, Map arguments, api.Diagnostic kind) {
      ...
      switch (kind) {
        ...
        case api.Diagnostic.HINT:
          ...
          if (!inUserCode(element, assumeInUserCode: true)) {
            ...
            SuppressionInfo info = suppressedWarnings.putIfAbsent(uri, () => new SuppressionInfo());
    ...
    reportDiagnostic(node, messageKind.message(arguments, terseDiagnostics), kind);
  }

  void reportDiagnostic(Spannable span, Message message, api.Diagnostic kind);

  void reportAssertionFailure(SpannableAssertionFailure ex) {
    String message = (ex.message != null) ? tryToString(ex.message) : tryToString(ex);
    SourceSpan span = spanFromSpannable(ex.node);
    reportDiagnosticInternal(ex.node, MessageKind.GENERIC, {
      'text': message
    }, api.Diagnostic.CRASH);
  }
  ...
  SourceSpan spanFromTokens(Token begin, Token end, [Uri uri]) {
    ...
    return SourceSpan.withCharacterOffsets(begin, end, (beginOffset, endOffset) => new SourceSpan(uri, beginOffset, endOffset));
  }
  ...
  SourceSpan spanFromElement(Element element) {
    ...
    if (element != null && element.position == null && !element.isLibrary && !element.isCompilationUnit) {
    ...
    return (position == null) ? new SourceSpan(uri, 0, 0) : spanFromTokens(position, position, uri);
  }
   ...
   */
  Uri translateResolvedUri(LibraryElement importingLibrary, Uri resolvedUri, Node node) {
...
class SourceSpan implements Spannable {
  ...
  static withCharacterOffsets(Token begin, Token end, f(int beginOffset, int endOffset)) {
...
assertDebugMode(String message) {
  assert(invariant(NO_LOCATION_SPANNABLE, DEBUG_MODE, message: 'Debug mode is not enabled: $message'));
}
...
bool invariant(Spannable spannable, var condition, {var message: null}) {
  ...
  if (spannable == null) {
    throw new SpannableAssertionFailure(CURRENT_ELEMENT_SPANNABLE, "Spannable was null for invariant. Use CURRENT_ELEMENT_SPANNABLE.");
  }
