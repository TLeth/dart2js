...
class FlatTypeMask implements TypeMask {
  ...
  bool isSingleImplementationOf(ClassElement cls, ClassWorld classWorld) {
    ...
    if (containsOnlyString(classWorld)) {
      return cls == classWorld.stringClass || cls == backend.stringImplementation;
    }
    ...
    if (containsOnlyInt(classWorld)) {
      return cls == classWorld.intClass || cls == backend.intImplementation || cls == backend.positiveIntImplementation || cls == backend.uint32Implementation || cls == backend.uint31Implementation;
    }
    if (containsOnlyDouble(classWorld)) {
      return cls == classWorld.doubleClass || cls == backend.doubleImplementation;
    }
    return false;
  }
  ...
  bool isInMask(TypeMask other, ClassWorld classWorld) {
    ...
    if (flatOther.isExact) {
      return (isExact && base == otherBase) || isSingleImplementationOf(otherBase, classWorld);
    }
  ...
  bool containsOnlyInt(ClassWorld classWorld) {
    Backend backend = classWorld.backend;
    return base == classWorld.intClass || base == backend.intImplementation || base == backend.positiveIntImplementation || base == backend.uint31Implementation || base == backend.uint32Implementation;
  }

  bool containsOnlyDouble(ClassWorld classWorld) {
    Backend backend = classWorld.backend;
    return base == classWorld.doubleClass || base == backend.doubleImplementation;
  }

  bool containsOnlyNum(ClassWorld classWorld) {
    Backend backend = classWorld.backend;
    return containsOnlyInt(classWorld) || containsOnlyDouble(classWorld) || base == classWorld.numClass || base == backend.numImplementation;
  }
  ...
  bool containsOnlyString(ClassWorld classWorld) {
    Backend backend = classWorld.backend;
    return base == classWorld.stringClass || base == backend.stringImplementation;
  }
  ...
  TypeMask unionSame(FlatTypeMask other, ClassWorld classWorld) {
    ...
    int combined = (flags > other.flags) ? flags | (other.flags & 1) : other.flags | (flags & 1);
    if (flags == combined) {
      return this;
  ...
  TypeMask unionStrictSubclass(FlatTypeMask other, ClassWorld classWorld) {
    ...
    if ((isExact && other.isExact) || base == classWorld.objectClass) {
      ...
      // constraining kind (the highest) of the two. If either one of
      // the masks are nullable the result should be nullable too.
      combined = (flags > other.flags) ? flags | (other.flags & 1) : other.flags | (flags & 1);
    }
    // If we weaken the constraint on this type, we have to make sure that
    // the result is normalized.
    return (flags != combined) ? (combined >> 1 == flags >> 1) ? new FlatTypeMask.internal(base, combined) : new FlatTypeMask.normalized(base, combined, classWorld) : this;
  }
  ...
  TypeMask unionStrictSubtype(FlatTypeMask other, ClassWorld classWorld) {
    ...
    // to normalize.
    return (flags != combined) ? new FlatTypeMask.internal(base, combined) : this;
  }
  ...
  TypeMask intersectionSame(FlatTypeMask other) {
    ...
    int combined = (flags < other.flags) ? flags & ((other.flags & 1) | ~1) : other.flags & ((flags & 1) | ~1);
    if (flags == combined) {
      return this;
    } else if (other.flags == combined) {
      return other;
  ...
  TypeMask intersectionStrictSubtype(FlatTypeMask other, ClassWorld classWorld) {
    assert(base != other.base);
    assert(classWorld.isSubtypeOf(other.base, base));
  ...
  TypeMask intersectionEmpty(FlatTypeMask other) {
    return isNullable && other.isNullable ? new TypeMask.empty() : new TypeMask.nonNullEmpty();
  }
   ...
   */
  static bool hasElementIn(ClassElement cls, Selector selector, Element element) {
    // Use [:implementation:] of [element]
    // because our function set only stores declarations.
    Element result = findMatchIn(cls, selector);
    return result == null ? false : result.implementation == element.implementation;
  }
  ...
  bool canHit(Element element, Selector selector, ClassWorld classWorld) {
    ...
    if (other == backend.nullImplementation) {
      ...
      return hasElementIn(self, selector, element) || other.isSubclassOf(self) || classWorld.hasAnySubclassThatMixes(self, other);
    } else {
      assert(isSubtype);
      assert(classWorld.isClosed);
      bool result = hasElementIn(self, selector, element) || other.implementsInterface(self) || classWorld.hasAnySubclassThatImplements(other, base) || classWorld.hasAnySubclassOfMixinUseThatImplements(other, base);
      ...
      return mixinUses.any((mixinApplication) => hasElementIn(mixinApplication, selector, element) || other.isSubclassOf(mixinApplication) || classWorld.hasAnySubclassThatMixes(mixinApplication, other));
    }
  }
   ...
   */
  static bool hasConcreteMatch(ClassElement cls, Selector selector, World world) {
    assert(invariant(cls, world.compiler.resolverWorld.isInstantiated(cls), message: '$cls has not been instantiated.'));
  ...
  bool needsNoSuchMethodHandling(Selector selector, ClassWorld classWorld) {
    ...
    return subclassesToCheck != null && subclassesToCheck.any(needsNoSuchMethod);
  }
  ...
  static Set<ClassElement> commonContainedClasses(FlatTypeMask x, FlatTypeMask y, ClassWorld classWorld) {
    ...
    Iterable<ClassElement> smallSet;
    Iterable<ClassElement> largeSet;
  ...
  static Iterable<ClassElement> containedSubset(FlatTypeMask x, ClassWorld classWorld) {
