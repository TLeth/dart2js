...
import 'constants/values.dart' show ConstantValue, ConstructedConstantValue, ListConstantValue, StringConstantValue, TypeConstantValue;

import 'dart_types.dart' show DartType, InterfaceType, TypeKind;

import 'dart2jslib.dart' show Compiler, CompilerTask, ConstantCompiler, MessageKind, TreeElements, invariant;

import 'elements/elements.dart' show ClassElement, Element, LibraryElement, MetadataAnnotation, ScopeContainerElement, VariableElement;

import 'tree/tree.dart' show Import, LibraryTag, NamedArgument, NewExpression, Node;
...
class MirrorUsageAnalyzerTask extends CompilerTask {
  ...
  void analyzeUsage(LibraryElement mainApp) {
    ...
    compiler.backend.registerMirrorUsage(symbols == null ? null : new Set<String>.from(symbols), targets == null ? null : new Set<Element>.from(targets), metaTargets == null ? null : new Set<Element>.from(metaTargets));
  ...
  bool hasMirrorUsage(Element element) {
    ...
    return library.isInternalLibrary || (librariesWithUsage != null && librariesWithUsage.contains(library));
  }
  ...
  void validate(NewExpression node, TreeElements mapping) {
    for (Node argument in node.send.arguments) {
      ...
      ConstantValue value = constantCompiler.compileNode(named.expression, mapping).value;

      MirrorUsageBuilder builder = new MirrorUsageBuilder(analyzer, mapping.analyzedElement.library, named.expression, value, mapping);

      if (named.name.source == 'symbols') {
        analyzer.cachedStrings[value] = builder.convertConstantToUsageList(value, onlyStrings: true);
      } else if (named.name.source == 'targets') {
        analyzer.cachedElements[value] = builder.resolveUsageList(builder.convertConstantToUsageList(value));
...
class MirrorUsageAnalyzer {
  ...
  void run() {
    ...
    Map<LibraryElement, List<MirrorUsage>> usageMap = collectMirrorsUsedAnnotation();
  ...
  Map<LibraryElement, List<MirrorUsage>> collectMirrorsUsedAnnotation() {
    Map<LibraryElement, List<MirrorUsage>> result = new Map<LibraryElement, List<MirrorUsage>>();
    for (LibraryElement library in compiler.libraryLoader.libraries) {
      ...
      for (LibraryTag tag in library.tags) {
        ...
        compiler.withCurrentElement(library, () {
          List<MirrorUsage> usages = mirrorsUsedOnLibraryTag(library, importTag);
          if (usages != null) {
  ...
  void propagateOverrides(Map<LibraryElement, List<MirrorUsage>> usageMap) {
    Map<LibraryElement, List<MirrorUsage>> propagatedOverrides = new Map<LibraryElement, List<MirrorUsage>>();
        ...
        if (override == wildcard) {
          for (LibraryElement overridden in wildcard) {
            if (overridden != library) {
              List<MirrorUsage> overriddenUsages = propagatedOverrides.putIfAbsent(overridden, () => <MirrorUsage>[]);
    ...
    propagatedOverrides.forEach((LibraryElement overridden, List<MirrorUsage> overriddenUsages) {
      List<MirrorUsage> usages = usageMap.putIfAbsent(overridden, () => <MirrorUsage>[]);
  ...
  List<MirrorUsage> mirrorsUsedOnLibraryTag(LibraryElement library, Import tag) {
  ...
  MirrorUsage merge(MirrorUsage a, MirrorUsage b) {
    ...
    if (a.symbols == null && a.targets == null && a.metaTargets == null) {
      return b;
    } else if (b.symbols == null && b.targets == null && b.metaTargets == null) {
      return a;
    }
  ...
  MirrorUsage buildUsage(ConstructedConstantValue constant) {
    ...
    VariableElement symbolsField = compiler.mirrorsUsedClass.lookupLocalMember('symbols');
    VariableElement targetsField = compiler.mirrorsUsedClass.lookupLocalMember('targets');
    VariableElement metaTargetsField = compiler.mirrorsUsedClass.lookupLocalMember('metaTargets');
    VariableElement overrideField = compiler.mirrorsUsedClass.lookupLocalMember('override');

    return new MirrorUsage(cachedStrings[fields[symbolsField]], cachedElements[fields[targetsField]], cachedElements[fields[metaTargetsField]], cachedElements[fields[overrideField]]);
  }
}
...
class MirrorUsage {
  ...
  String toString() {
    return 'MirrorUsage(' 'symbols = $symbols, ' 'targets = $targets, ' 'metaTargets = $metaTargets, ' 'override = $override' ')';
...
class MirrorUsageBuilder {
  ...
  MirrorUsageBuilder(this.analyzer, this.enclosingLibrary, this.spannable, this.constant, this.elements);
  ...
  List convertConstantToUsageList(ConstantValue constant, {bool onlyStrings: false}) {
    if (constant.isNull) {
      return null;
      ...
      for (ConstantValue entry in list.entries) {
        if (entry.isString) {
          ...
          MessageKind kind = onlyStrings ? MessageKind.MIRRORS_EXPECTED_STRING : MessageKind.MIRRORS_EXPECTED_STRING_OR_TYPE;
          compiler.reportHint(node, kind, {
            'name': node,
            'type': apiTypeOf(entry)
          });
        }
      }
      return result;
      ...
      var iterable = string.primitiveValue.slowToString().split(',').map((e) => e.trim());
      return onlyStrings ? new List<String>.from(iterable) : iterable.toList();
    } else {
      Spannable node = positionOf(constant);
      MessageKind kind = onlyStrings ? MessageKind.MIRRORS_EXPECTED_STRING_OR_LIST : MessageKind.MIRRORS_EXPECTED_STRING_TYPE_OR_LIST;
      compiler.reportHint(node, kind, {
        'name': node,
        'type': apiTypeOf(constant)
      });
      return null;
    }
  }
  ...
  DartType apiTypeOf(ConstantValue constant) {
    ...
    if (type.isInterfaceType && library.isInternalLibrary) {
      ...
      for (DartType supertype in cls.allSupertypes) {
        if (supertype.isInterfaceType && !supertype.element.library.isInternalLibrary) {
    ...
    for (var entry in list) {
      if (entry is DartType) {
        ...
        if (libraryNameCandiate == string) {
          ...
          e = resolveLocalExpression(libraryCandiate, string.substring(libraryNameCandiate.length + 1).split('.'));
        } else {
  ...
  Element resolveExpression(String expression) {
    ...
    if (element == null) {
      compiler.reportHint(spannable, MessageKind.MIRRORS_CANNOT_RESOLVE_IN_CURRENT_LIBRARY, {
        'name': expression
      });
      return null;
    } else {
    ...
    for (String identifier in identifiers) {
      ...
      if (e == null) {
        if (current.isLibrary) {
          ...
          compiler.reportHint(spannable, MessageKind.MIRRORS_CANNOT_RESOLVE_IN_LIBRARY, {
            'name': identifiers[0],
            'library': library.getLibraryOrScriptName()
          });
        } else {
          compiler.reportHint(spannable, MessageKind.MIRRORS_CANNOT_FIND_IN_ELEMENT, {
            'name': identifier,
