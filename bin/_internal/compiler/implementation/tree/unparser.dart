...
class Unparser extends Indentation implements Visitor {
  ...
  void addToken(Token token) {
    ...
    write(token.value);
    if (identical(token.kind, KEYWORD_TOKEN) || identical(token.kind, IDENTIFIER_TOKEN)) {
      write(' ');
    }
  }
  ...
  unparseBlockStatements(NodeList statements) {
    ...
    if (nodes != null && !nodes.isEmpty) {
      ...
      newline();
      visit(nodes.head);
      String delimiter = (statements.delimiter == null) ? "" : "${statements.delimiter}";
  ...
  visitFunctionExpression(FunctionExpression node) {
    ...
    if (node.initializers != null) {
      ...
      write(':');
      space();
      unparseNodeListFrom(node.initializers, node.initializers.nodes, spaces: true);
    }
  ...
  visitSend(Send node) {
    ...
    bool spacesNeeded = minify ? identical(opString, 'is') || identical(opString, 'as') : (opString != null && !node.isPrefix && !node.isIndex);
  ...
  unparseImportTag(String uri, {String prefix, List<String> shows: const <String>[], bool isDeferred: false}) {
