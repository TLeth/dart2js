...
abstract class TypeMask {
  factory TypeMask(ClassElement base, int kind, bool isNullable, ClassWorld classWorld) {
    return new FlatTypeMask.normalized(base, (kind << 1) | (isNullable ? 1 : 0), classWorld);
  }

  const factory TypeMask.empty() = FlatTypeMask.empty;

  factory TypeMask.exact(ClassElement base, ClassWorld classWorld) {
    assert(invariant(base, classWorld.isInstantiated(base), message: "Cannot create extact type mask for uninstantiated class"));
  ...
  static bool isNormalized(TypeMask mask, ClassWorld classWorld) {
    ...
    if (mask is FlatTypeMask) {
      ...
      return classWorld.hasAnySubtype(mask.base) && !classWorld.hasOnlySubclasses(mask.base);
