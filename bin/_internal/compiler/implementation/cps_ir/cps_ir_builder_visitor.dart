...
class IrBuilderTask extends CompilerTask {
  final Map<Element, ir.FunctionDefinition> nodes = <Element, ir.FunctionDefinition>{};
      ...
      resolved.forEach((AstElement element) {
        if (canBuild(element)) {
          ...
          compiler.withCurrentElement(element, () {
            ...
            IrBuilderVisitor builder = new IrBuilderVisitor(elementsMapping, compiler, sourceFile);
  ...
  bool irEnabled({bool useNewBackend: false}) {
    ...
    return (useNewBackend || const bool.fromEnvironment('USE_NEW_BACKEND')) && compiler.backend is DartBackend && !compiler.enableTypeAssertions && !compiler.enableConcreteTypeInference;
  }
 ...
 * an expression.
 */
class IrBuilderVisitor extends ResolvedVisitor<ir.Primitive> with IrBuilderMixin {
  ...
  ir.FunctionDefinition buildFunctionInternal(FunctionElement element) {
    ...
    return withBuilder(new IrBuilder(compiler.backend.constantSystem, element, closureLocals.usedFromClosure), () {
      FunctionSignature signature = element.functionSignature;
      signature.orderedForEachParameter((ParameterElement parameterElement) {
        irBuilder.createParameter(parameterElement, isClosureVariable: isClosureVariable(parameterElement));
      });
  ...
  void invokeFullJoin(ir.Continuation join, JumpCollector jumps, {recursive: false}) {
    ...
    for (int i = 0; i < jumps.length; ++i) {
      ...
      invoke.arguments = new List<ir.Reference>.generate(join.parameters.length, (i) => new ir.Reference(currentEnvironment[i]));
  ...
  ir.Primitive visitFor(ast.For node) {
    ...
    if (node.condition == null) {
      ...
      condition = irBuilder.makePrimConst(irBuilder.state.constantSystem.createBool(true));
    ...
    IrBuilder updateBuilder = hasContinues ? new IrBuilder.recursive(condBuilder) : bodyBuilder;
    ...
    ir.LetCont branch = new ir.LetCont(exitContinuation, new ir.LetCont(bodyContinuation, new ir.Branch(new ir.IsTrue(condition), bodyContinuation, exitContinuation)));
    ...
    ir.Continuation loopContinuation = new ir.Continuation(condBuilder._parameters);
    ...
    if (hasContinues) {
      ...
      bodyContinuation.body = new ir.LetCont(continueContinuation, bodyBuilder._root);
    } else {
      bodyContinuation.body = bodyBuilder._root;
    }

    loopContinuation.body = condBuilder._root;
    irBuilder.add(new ir.LetCont(loopContinuation, new ir.InvokeContinuation(loopContinuation, irBuilder.environment.index2value)));
    if (hasBreaks) {
      irBuilder._current = branch;
      irBuilder.environment = condBuilder.environment;
      breakCollector.addJump(irBuilder);
      letJoin.continuation = irBuilder.createJoin(irBuilder.environment.length, breakCollector);
  ...
  ir.Primitive visitIf(ast.If node) {
    ...
    ir.Expression letElse = new ir.LetCont(elseContinuation, new ir.Branch(new ir.IsTrue(condition), thenContinuation, elseContinuation));
    ...
    if (thenBuilder.isOpen && elseBuilder.isOpen) {
      ...
      joinContinuation = irBuilder.createJoin(irBuilder.environment.length, jumps);
    ...
    if (joinContinuation == null) {
      ...
      if (thenBuilder.isOpen) {
        irBuilder._current = (thenBuilder._root == null) ? letThen : thenBuilder._current;
        irBuilder.environment = thenBuilder.environment;
      } else if (elseBuilder.isOpen) {
        irBuilder._current = (elseBuilder._root == null) ? letElse : elseBuilder._current;
  ...
  ir.Primitive visitLabeledStatement(ast.LabeledStatement node) {
    ...
    return body is ast.Loop ? visit(body) : giveup(node, 'labeled statement');
  }
  ...
  ir.Primitive visitWhile(ast.While node) {
    ...
    IrBuilder condBuilder = new IrBuilder.recursive(irBuilder);
    ir.Primitive condition = withBuilder(condBuilder, () => visit(node.condition));
  ...
  ir.Primitive visitForIn(ast.ForIn node) {
    ...
    irBuilder.add(new ir.LetCont(iteratorInvoked, new ir.InvokeMethod(expressionReceiver, new Selector.getter("iterator", null), iteratorInvoked, emptyArguments)));

    ir.Parameter condition = new ir.Parameter(null);
    ir.Continuation moveNextInvoked = new ir.Continuation([condition]);
    condBuilder.add(new ir.LetCont(moveNextInvoked, new ir.InvokeMethod(iterator, new Selector.call("moveNext", null, 0), moveNextInvoked, emptyArguments)));
    ...
    bodyBuilder.add(new ir.LetCont(currentInvoked, new ir.InvokeMethod(iterator, new Selector.getter("current", null), currentInvoked, emptyArguments)));
    if (Elements.isLocal(variableElement)) {
      withBuilder(bodyBuilder, () => setLocal(variableElement, currentValue));
    } else if (Elements.isStaticOrTopLevel(variableElement)) {
      withBuilder(bodyBuilder, () => setStatic(variableElement, selector, currentValue));
    } else {
      ir.Primitive receiver = withBuilder(bodyBuilder, () => lookupThis());
      withBuilder(bodyBuilder, () => setDynamic(null, receiver, selector, currentValue));
    }
  ...
  ir.Primitive visitVariableDefinitions(ast.VariableDefinitions node) {
    ...
    if (node.modifiers.isConst) {
      ...
      for (ast.Node definition in node.definitions.nodes) {
        ...
        irBuilder.declareLocalVariable(element, initialValue: initialValue, isClosureVariable: isClosureVariable(element));
      }
    }
    return null;
  }
  ...
  ir.Primitive visitConditional(ast.Conditional node) {
    ...
    IrBuilder thenBuilder = new IrBuilder.delimited(irBuilder);
    IrBuilder elseBuilder = new IrBuilder.delimited(irBuilder);
    ir.Primitive thenValue = withBuilder(thenBuilder, () => visit(node.thenExpression));
    ir.Primitive elseValue = withBuilder(elseBuilder, () => visit(node.elseExpression));
    ...
    jumps.addJump(thenBuilder);
    jumps.addJump(elseBuilder);
    ir.Continuation joinContinuation = irBuilder.createJoin(irBuilder.environment.length + 1, jumps);
    ...
    thenContinuation.body = thenBuilder._root;
    elseContinuation.body = elseBuilder._root;
    irBuilder.add(new ir.LetCont(joinContinuation, new ir.LetCont(thenContinuation, new ir.LetCont(elseContinuation, new ir.Branch(new ir.IsTrue(condition), thenContinuation, elseContinuation)))));
    return (thenValue == elseValue) ? thenValue : joinContinuation.parameters.last;
  }
  ...
  ConstantExpression getConstantForNode(ast.Node node) {
    ConstantExpression constant = compiler.backend.constantCompilerTask.compileNode(node, elements);
  ...
  ConstantExpression getConstantForVariable(VariableElement element) {
    ...
    assert(invariant(element, constant != null, message: 'No constant computed for $element'));
    return constant;
  }
  ...
  ir.Primitive translateClosureCall(ir.Primitive receiver, Selector closureSelector, ast.NodeList arguments) {
    Selector namedCallSelector = new Selector(closureSelector.kind, "call", closureSelector.library, closureSelector.argumentCount, closureSelector.namedArguments);
    List<ir.Primitive> args = arguments.nodes.mapToList(visit, growable: false);
    return irBuilder.continueWithExpression((k) => new ir.InvokeMethod(receiver, namedCallSelector, k, args));
  }
  ...
  ir.Primitive visitClosureSend(ast.Send node) {
    ...
    return translateClosureCall(closureTarget, closureSelector, node.argumentsNode);
  }
  ...
  ir.Expression createDynamicInvoke(ast.Send node, Selector selector, ir.Definition receiver, ir.Continuation k, List<ir.Definition> arguments) {
    return node != null && node.receiver != null && node.receiver.isSuper() ? new ir.InvokeSuperMethod(selector, k, arguments) : new ir.InvokeMethod(receiver, selector, k, arguments);
  }
  ...
  _GetterElements translateGetter(ast.Send node, Selector selector) {
    ...
    } else if (element == null || Elements.isInstanceField(element) || Elements.isInstanceMethod(element) || selector.isIndex || // TODO(johnniwinther): clean up semantics of resolution.
    node.isSuperCall) {
      ...
      assert(selector.kind == SelectorKind.GETTER || selector.kind == SelectorKind.INDEX);
      result = irBuilder.continueWithExpression((k) => createDynamicInvoke(node, selector, receiver, k, arguments));
    } else if (element.isField || element.isGetter || element.isErroneous || element.isSetter) {
      ...
      assert(selector.kind == SelectorKind.GETTER || selector.kind == SelectorKind.SETTER);
    ...
    return new _GetterElements(result: result, index: index, receiver: receiver);
  }
  ...
  ir.Primitive translateLogicalOperator(ast.Operator op, ast.Expression left, ast.Expression right) {
    ...
    return irBuilder.buildLogicalOperator(leftValue, buildRightValue, isLazyOr: op.source == '||');
  }
  ...
  ir.Primitive visitOperatorSend(ast.Send node) {
    ...
    assert(invariant(node, op.source == "is" || op.source == "as", message: "unexpected operator $op"));
    DartType type = elements.getType(node.typeAnnotationFromIsCheckOrCast);
    ir.Primitive receiver = visit(node.receiver);
    ir.Primitive check = irBuilder.continueWithExpression((k) => new ir.TypeOperator(op.source, receiver, type, k));
  ...
  ir.Primitive visitStaticSend(ast.Send node) {
    ...
    List<ir.Definition> arguments = node.arguments.mapToList(visit, growable: false);
  ...
  void setStatic(Element element, Selector selector, ir.Primitive valueToStore) {
    assert(element.isErroneous || element.isField || element.isSetter);
    irBuilder.continueWithExpression((k) => new ir.InvokeStatic(element, selector, k, [valueToStore]));
  }

  void setDynamic(ast.Node node, ir.Primitive receiver, Selector selector, ir.Primitive valueToStore) {
    List<ir.Definition> arguments = [valueToStore];
    irBuilder.continueWithExpression((k) => createDynamicInvoke(node, selector, receiver, k, arguments));
  }

  void setIndex(ast.Node node, ir.Primitive receiver, Selector selector, ir.Primitive index, ir.Primitive valueToStore) {
  ...
  ir.Primitive visitSendSet(ast.SendSet node) {
    ...
    assert(// Indexing send-sets have an argument for the index.
    (selector.isIndexSet ? 1 : 0) + // Non-increment send-sets have one more argument.
    (ast.Operator.INCREMENT_OPERATORS.contains(op.source) ? 0 : 1) == node.argumentCount());

    ast.Node getAssignArgument() {
      assert(invariant(node, !node.arguments.isEmpty, message: "argument expected"));
      return selector.isIndexSet ? node.arguments.tail.head : node.arguments.head;
    }
    ...
    if (op.source == "=") {
      ...
      if (ast.Operator.INCREMENT_OPERATORS.contains(op.source)) {
        arg = irBuilder.makePrimConst(irBuilder.state.constantSystem.createInt(1));
      ...
      ir.Expression invoke = new ir.InvokeMethod(originalValue, operatorSelector, k, [arg]);
      irBuilder.add(new ir.LetCont(k, invoke));
    }

    if (Elements.isLocal(element)) {
      setLocal(element, valueToStore);
    } else if ((!node.isSuperCall && Elements.isErroneousElement(element)) || Elements.isStaticOrTopLevel(element)) {
      setStatic(element, elements.getSelector(node), valueToStore);
    } else {
      // Setter or index-setter invocation
      Selector selector = elements.getSelector(node);
      assert(selector.kind == SelectorKind.SETTER || selector.kind == SelectorKind.INDEX);
      if (selector.isIndexSet) {
  ...
  ir.Primitive visitNewExpression(ast.NewExpression node) {
    ...
    List<ir.Primitive> args = node.send.arguments.mapToList(visit, growable: false);
    return irBuilder.continueWithExpression((k) => new ir.InvokeConstructor(type, element, selector, k, args));
  }
  ...
  ir.Primitive visitStringJuxtaposition(ast.StringJuxtaposition node) {
    ...
    return irBuilder.continueWithExpression((k) => new ir.ConcatenateStrings(k, [first, second]));
  }
  ...
  ir.Primitive visitStringInterpolation(ast.StringInterpolation node) {
    ...
    return irBuilder.continueWithExpression((k) => new ir.ConcatenateStrings(k, arguments));
  }
  ...
  ir.FunctionDefinition makeSubFunction(ast.FunctionExpression node) {
    ...
    return new IrBuilderVisitor(elements, compiler, sourceFile).buildFunctionInternal(elements[node]);
  }
...
class DetectClosureVariables extends ast.Visitor {
  ...
  visitSend(ast.Send node) {
    Element element = elements[node];
    if (Elements.isLocal(element) && !element.isConst && element.enclosingElement != currentFunction) {
      LocalElement local = element;
