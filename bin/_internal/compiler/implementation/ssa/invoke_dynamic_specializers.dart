...
class InvokeDynamicSpecializer {
  ...
  TypeMask computeTypeFromInputTypes(HInvokeDynamic instruction, Compiler compiler) {
    Selector selector = instruction.selector;
    return TypeMaskFactory.inferredTypeForSelector(selector, compiler);
  }

  HInstruction tryConvertToBuiltin(HInvokeDynamic instruction, Compiler compiler) {
    return null;
  }
  ...
  static InvokeDynamicSpecializer lookupSpecializer(Selector selector) {
    if (selector.kind == SelectorKind.INDEX) {
      return selector.name == '[]' ? const IndexSpecializer() : const IndexAssignSpecializer();
...
class IndexAssignSpecializer extends InvokeDynamicSpecializer {
  ...
  HInstruction tryConvertToBuiltin(HInvokeDynamic instruction, Compiler compiler) {
    if (instruction.inputs[1].isMutableIndexable(compiler)) {
      if (!instruction.inputs[2].isInteger(compiler) && compiler.enableTypeAssertions) {
        // We want the right checked mode error.
        return null;
      }
      return new HIndexAssign(instruction.inputs[1], instruction.inputs[2], instruction.inputs[3], instruction.selector);
    }
    return null;
  }
}
...
class IndexSpecializer extends InvokeDynamicSpecializer {
  ...
  HInstruction tryConvertToBuiltin(HInvokeDynamic instruction, Compiler compiler) {
    ...
    TypeMask receiverType = instruction.getDartReceiver(compiler).instructionType;
    Selector refined = new TypedSelector(receiverType, instruction.selector, compiler.world);
    TypeMask type = TypeMaskFactory.inferredTypeForSelector(refined, compiler);
    return new HIndex(instruction.inputs[1], instruction.inputs[2], instruction.selector, type);
  }
}
...
class BitNotSpecializer extends InvokeDynamicSpecializer {
  ...
  HInstruction tryConvertToBuiltin(HInvokeDynamic instruction, Compiler compiler) {
    ...
    if (input.isNumber(compiler)) {
      return new HBitNot(input, instruction.selector, computeTypeFromInputTypes(instruction, compiler));
    }
    return null;
  }
}
...
abstract class BinaryArithmeticSpecializer extends InvokeDynamicSpecializer {
  ...
  bool isBuiltin(HInvokeDynamic instruction, Compiler compiler) {
    return instruction.inputs[1].isNumber(compiler) && instruction.inputs[2].isNumber(compiler);
  }
  ...
  bool inputsArePositiveIntegers(HInstruction instruction, Compiler compiler) {
    ...
    HInstruction right = instruction.inputs[2];
    JavaScriptBackend backend = compiler.backend;
    return left.isPositiveIntegerOrNull(compiler) && right.isPositiveIntegerOrNull(compiler);
  }

  HInstruction newBuiltinVariant(HInvokeDynamic instruction, Compiler compiler);

  Selector renameToOptimizedSelector(String name, Selector selector, Compiler compiler) {
    if (selector.name == name) return selector;
    JavaScriptBackend backend = compiler.backend;
    Selector newSelector = new Selector(SelectorKind.CALL, name, backend.interceptorsLibrary, selector.argumentCount);
    return selector.mask == null ? newSelector : new TypedSelector(selector.mask, newSelector, compiler.world);
  }
}
...
class AddSpecializer extends BinaryArithmeticSpecializer {
  ...
  HInstruction newBuiltinVariant(HInvokeDynamic instruction, Compiler compiler) {
    return new HAdd(instruction.inputs[1], instruction.inputs[2], instruction.selector, computeTypeFromInputTypes(instruction, compiler));
  }
}
...
class DivideSpecializer extends BinaryArithmeticSpecializer {
  ...
  TypeMask computeTypeFromInputTypes(HInstruction instruction, Compiler compiler) {
    HInstruction left = instruction.inputs[1];
    JavaScriptBackend backend = compiler.backend;
    if (left.isNumberOrNull(compiler)) {
      return backend.doubleType;
  ...
  HInstruction newBuiltinVariant(HInvokeDynamic instruction, Compiler compiler) {
    JavaScriptBackend backend = compiler.backend;
    return new HDivide(instruction.inputs[1], instruction.inputs[2], instruction.selector, backend.doubleType);
  }
}
...
class MultiplySpecializer extends BinaryArithmeticSpecializer {
  ...
  HInstruction newBuiltinVariant(HInvokeDynamic instruction, Compiler compiler) {
    return new HMultiply(instruction.inputs[1], instruction.inputs[2], instruction.selector, computeTypeFromInputTypes(instruction, compiler));
  }
}
...
class SubtractSpecializer extends BinaryArithmeticSpecializer {
  ...
  HInstruction newBuiltinVariant(HInvokeDynamic instruction, Compiler compiler) {
    return new HSubtract(instruction.inputs[1], instruction.inputs[2], instruction.selector, computeTypeFromInputTypes(instruction, compiler));
  }
}
...
class TruncatingDivideSpecializer extends BinaryArithmeticSpecializer {
  ...
  HInstruction tryConvertToBuiltin(HInvokeDynamic instruction, Compiler compiler) {
    ...
    if (isBuiltin(instruction, compiler)) {
      if (right.isPositiveInteger(compiler) && isNotZero(right, compiler)) {
        ...
        // and not 0, nor -1.
        instruction.selector = renameToOptimizedSelector('_tdivFast', instruction.selector, compiler);
      }
      clearAllSideEffects(instruction);
    }
    return null;
  ...
  HInstruction newBuiltinVariant(HInvokeDynamic instruction, Compiler compiler) {
    return new HTruncatingDivide(instruction.inputs[1], instruction.inputs[2], instruction.selector, computeTypeFromInputTypes(instruction, compiler));
  }
}
...
class ShiftLeftSpecializer extends BinaryBitOpSpecializer {
  ...
  HInstruction tryConvertToBuiltin(HInvokeDynamic instruction, Compiler compiler) {
    ...
    if (left.isNumber(compiler)) {
      ...
      if (isPositive(right, compiler)) {
        instruction.selector = renameToOptimizedSelector('_shlPositive', instruction.selector, compiler);
      }
    }
    return null;
  }
  ...
  HInstruction newBuiltinVariant(HInvokeDynamic instruction, Compiler compiler) {
    JavaScriptBackend backend = compiler.backend;
    return new HShiftLeft(instruction.inputs[1], instruction.inputs[2], instruction.selector, computeTypeFromInputTypes(instruction, compiler));
  }
}
...
class ShiftRightSpecializer extends BinaryBitOpSpecializer {
  ...
  HInstruction tryConvertToBuiltin(HInvokeDynamic instruction, Compiler compiler) {
    ...
    if (left.isNumber(compiler)) {
      ...
      if (isPositive(right, compiler) && isPositive(left, compiler)) {
        instruction.selector = renameToOptimizedSelector('_shrBothPositive', instruction.selector, compiler);
      } else if (isPositive(left, compiler) && right.isNumber(compiler)) {
        instruction.selector = renameToOptimizedSelector('_shrReceiverPositive', instruction.selector, compiler);
      } else if (isPositive(right, compiler)) {
        instruction.selector = renameToOptimizedSelector('_shrOtherPositive', instruction.selector, compiler);
      }
    }
    return null;
  }
  ...
  HInstruction newBuiltinVariant(HInvokeDynamic instruction, Compiler compiler) {
    JavaScriptBackend backend = compiler.backend;
    return new HShiftRight(instruction.inputs[1], instruction.inputs[2], instruction.selector, computeTypeFromInputTypes(instruction, compiler));
  }
...
class BitOrSpecializer extends BinaryBitOpSpecializer {
  ...
  HInstruction newBuiltinVariant(HInvokeDynamic instruction, Compiler compiler) {
    JavaScriptBackend backend = compiler.backend;
    return new HBitOr(instruction.inputs[1], instruction.inputs[2], instruction.selector, computeTypeFromInputTypes(instruction, compiler));
  }
}
...
class BitAndSpecializer extends BinaryBitOpSpecializer {
  ...
  HInstruction newBuiltinVariant(HInvokeDynamic instruction, Compiler compiler) {
    JavaScriptBackend backend = compiler.backend;
    return new HBitAnd(instruction.inputs[1], instruction.inputs[2], instruction.selector, computeTypeFromInputTypes(instruction, compiler));
  }
}
...
class BitXorSpecializer extends BinaryBitOpSpecializer {
  ...
  HInstruction newBuiltinVariant(HInvokeDynamic instruction, Compiler compiler) {
    JavaScriptBackend backend = compiler.backend;
    return new HBitXor(instruction.inputs[1], instruction.inputs[2], instruction.selector, computeTypeFromInputTypes(instruction, compiler));
  }
}
...
class EqualsSpecializer extends RelationalSpecializer {
  ...
  HInstruction tryConvertToBuiltin(HInvokeDynamic instruction, Compiler compiler) {
    ...
    Selector selector = new TypedSelector(instructionType, instruction.selector, world);
    JavaScriptBackend backend = compiler.backend;
  ...
  HInstruction newBuiltinVariant(HInvokeDynamic instruction, Compiler compiler) {
    JavaScriptBackend backend = compiler.backend;
    return new HIdentity(instruction.inputs[1], instruction.inputs[2], instruction.selector, backend.boolType);
  }
}
...
class LessSpecializer extends RelationalSpecializer {
  ...
  HInstruction newBuiltinVariant(HInvokeDynamic instruction, Compiler compiler) {
    JavaScriptBackend backend = compiler.backend;
    return new HLess(instruction.inputs[1], instruction.inputs[2], instruction.selector, backend.boolType);
  }
}
...
class GreaterSpecializer extends RelationalSpecializer {
  ...
  HInstruction newBuiltinVariant(HInvokeDynamic instruction, Compiler compiler) {
    JavaScriptBackend backend = compiler.backend;
    return new HGreater(instruction.inputs[1], instruction.inputs[2], instruction.selector, backend.boolType);
  }
}
...
class GreaterEqualSpecializer extends RelationalSpecializer {
  ...
  HInstruction newBuiltinVariant(HInvokeDynamic instruction, Compiler compiler) {
    JavaScriptBackend backend = compiler.backend;
    return new HGreaterEqual(instruction.inputs[1], instruction.inputs[2], instruction.selector, backend.boolType);
  }
}
...
class LessEqualSpecializer extends RelationalSpecializer {
  ...
  HInstruction newBuiltinVariant(HInvokeDynamic instruction, Compiler compiler) {
    JavaScriptBackend backend = compiler.backend;
    return new HLessEqual(instruction.inputs[1], instruction.inputs[2], instruction.selector, backend.boolType);
  }
}
