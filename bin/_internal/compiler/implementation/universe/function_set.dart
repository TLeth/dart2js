...
class FunctionSet {
  ...
  FunctionSetQuery query(Selector selector) {
    ...
    selector = (selector.mask == null) ? compiler.noSuchMethodSelector : new TypedSelector(selector.mask, compiler.noSuchMethodSelector, compiler.world);
...
class FunctionSetNode {
  final String name;
  final Map<Selector, FunctionSetQuery> cache = new Map<Selector, FunctionSetQuery>();
  ...
  TypeMask getNonNullTypeMaskOfSelector(Selector selector, Compiler compiler) {
    ...
    return selector.mask != null ? selector.mask : new TypeMask.subclass(compiler.objectClass, compiler.world);
  }

  FunctionSetQuery query(Selector selector, Compiler compiler, FunctionSetNode noSuchMethods) {
    ...
    // potential targets.
    if (noSuchMethods != null && mask.needsNoSuchMethodHandling(selector, classWorld)) {
      FunctionSetQuery noSuchMethodQuery = noSuchMethods.query(new TypedSelector(mask, compiler.noSuchMethodSelector, classWorld), compiler, null);
    ...
    cache[selector] = result = (functions != null) ? newQuery(functions, selector, compiler) : const FunctionSetQuery(const <Element>[]);
    return result;
  }

  FunctionSetQuery newQuery(Iterable<Element> functions, Selector selector, Compiler compiler) {
