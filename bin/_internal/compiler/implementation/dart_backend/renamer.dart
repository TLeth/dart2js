...
class GlobalRenamable extends Renamable {
  ...
  int compareInternals(GlobalRenamable other) => compareElements(this.entity, other.entity);
...
class MemberRenamable extends Renamable {
  ...
  MemberRenamable(this.identifier, Set<Node> nodes)
      : super(nodes);
  int compareInternals(MemberRenamable other) => this.identifier.compareTo(other.identifier);
...
class LocalRenamable extends Renamable {
  LocalRenamable(Set<Node> nodes)
      : super(nodes);
  int compareInternals(LocalRenamable other) => _compareNodes(sorted(this.nodes, _compareNodes)[0], sorted(other.nodes, _compareNodes)[0]);
...
class PlaceholderRenamer {
  ...
  final Map<Entity, Map<String, String>> _privateCache = new Map<Entity, Map<String, String>>();
  ...
  PlaceholderRenamer(this.fixedMemberNames, this.reexportingLibraries, {this.enableMinification, this.cutDeclarationTypes});
  ...
  String _generateUniqueTopLevelName(originalName) {
    String newName = _generator.generate(originalName, (name) {
      return _forbiddenIdentifiers.contains(name) || _allNamedParameterIdentifiers.contains(name);
    });
  ...
  String _getPrivateName(LibraryElement library, String originalName) {
    return _privateCache.putIfAbsent(library, () => new Map<String, String>()).putIfAbsent(originalName, () => _generateUniqueTopLevelName(originalName));
  }
  ...
  String _renameGlobal(Entity entity) {
    assert(entity is! Element || Elements.isErroneousElement(entity) || Elements.isStaticOrTopLevel(entity) || entity is TypeVariableElement);
    ...
    if (entity is Element) {
      ...
      if (library.isPlatformLibrary) {
        ...
        if (library.isInternalLibrary) {
          throw new SpannableAssertionFailure(entity, "Internal library $library should never have been imported from " "the code compiled by dart2dart.");
        }
        return entity.name;
      }
    }
    String name = _renamedCache.putIfAbsent(entity, () => _generateUniqueTopLevelName(entity.name));
  ...
  void _computeMinifiedRenames(PlaceholderCollector placeholderCollector) {
    ...
    int maxLength = placeholderCollector.functionScopes.values.fold(0, (a, b) => max(a, b.localPlaceholders.length));

    List<Set<Node>> allLocals = new List<Set<Node>>.generate(maxLength, (_) => new Set<Node>());

    for (FunctionScope functionScope in placeholderCollector.functionScopes.values) {
      // Add current sorted local identifiers to the whole sorted list
      // of all local identifiers for all functions.
      List<LocalPlaceholder> currentSortedPlaceholders = sorted(functionScope.localPlaceholders, compareBy((LocalPlaceholder ph) => -ph.nodes.length));

      List<Set<Node>> currentSortedNodes = currentSortedPlaceholders.map((LocalPlaceholder ph) => ph.nodes).toList();
    ...
    placeholderCollector.elementNodes.forEach((Element element, Set<Node> nodes) {
      renamables.add(new GlobalRenamable(element, nodes));
    });
    placeholderCollector.memberPlaceholders.forEach((String memberName, Set<Identifier> identifiers) {
  ...
  void _computeNonMinifiedRenames(PlaceholderCollector placeholderCollector) {
    ...
    // Rename locals.
    placeholderCollector.functionScopes.forEach((functionElement, functionScope) {
      ...
      for (LocalPlaceholder placeholder in placeholders) {
        String nextId = _generator.generate(placeholder.identifier, (name) {
          return functionScope.parameterIdentifiers.contains(name) || _forbiddenIdentifiers.contains(name) || usedLocalIdentifiers.contains(name) || memberIdentifiers.contains(name);
        });
  ...
  void computeRenames(PlaceholderCollector placeholderCollector) {
    ...
    // Rename constructors.
    for (ConstructorPlaceholder placeholder in placeholderCollector.constructorPlaceholders) {
      renames[placeholder.node] = _renameConstructor(placeholder);
    }
    ;

    // Rename private identifiers uniquely for each library.
    placeholderCollector.privateNodes.forEach((LibraryElement library, Set<Identifier> identifiers) {
    ...
    if (cutDeclarationTypes) {
      for (DeclarationTypePlaceholder placeholder in placeholderCollector.declarationTypePlaceholders) {
...
String generateMiniId(int index) {
  const String firstCharAlphabet = r'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const String otherCharsAlphabet = r'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_$';
  ...
  resultBuilder.writeCharCode(firstCharAlphabet.codeUnitAt(index % firstCharAlphabet.length));
