...
/// element is `null`.
const Spannable CURRENT_ELEMENT_SPANNABLE = const _SpannableSentinel("Current element");
...
class SpannableAssertionFailure {
  ...
  String toString() => 'Assertion failure' '${message != null ? ': $message' : ''}';
}
...
void writeJsonEscapedCharsOn(String string, buffer) {
  ...
  for (int i = 0; i < string.length; i++) {
    int code = string.codeUnitAt(i);
    if (code < 0x20 || code == $DEL || code == $DQ || code == $LS || code == $PS || code == $BACKSLASH || code >= 0x80) {
...
int computeHashCode(part1, [part2, part3, part4, part5]) {
  return (part1.hashCode ^ part2.hashCode ^ part3.hashCode ^ part4.hashCode ^ part5.hashCode) & 0x3fffffff;
}

String modifiersToString({bool isStatic: false, bool isAbstract: false, bool isFinal: false, bool isVar: false, bool isConst: false, bool isFactory: false, bool isExternal: false}) {
