...
abstract class TreeElements {
  ...
  ConstructorElement getRedirectingTargetConstructor(RedirectingFactoryBody node);
...
class TreeElementMapping implements TreeElements {
  ...
  void registerPotentialMutationIn(Node contextNode, VariableElement element, Node mutationNode) {
    if (_potentiallyMutatedIn == null) {
      _potentiallyMutatedIn = new Maplet<Node, Map<VariableElement, List<Node>>>();
    }
    Map<VariableElement, List<Node>> mutationMap = _potentiallyMutatedIn.putIfAbsent(contextNode, () => new Maplet<VariableElement, List<Node>>());
  ...
  void registerPotentialMutationInClosure(VariableElement element, Node mutationNode) {
    if (_potentiallyMutatedInClosure == null) {
      _potentiallyMutatedInClosure = new Maplet<VariableElement, List<Node>>();
    }
    _potentiallyMutatedInClosure.putIfAbsent(element, () => <Node>[]).add(mutationNode);
  }
  ...
  void setAccessedByClosureIn(Node contextNode, VariableElement element, Node accessNode) {
    if (_accessedByClosureIn == null) {
      _accessedByClosureIn = new Map<Node, Map<VariableElement, List<Node>>>();
    }
    Map<VariableElement, List<Node>> accessMap = _accessedByClosureIn.putIfAbsent(contextNode, () => new Maplet<VariableElement, List<Node>>());
...
class ResolverTask extends CompilerTask {
  ...
  TreeElements resolve(Element element) {
    return measure(() {
      ...
      if (identical(kind, ElementKind.GENERATIVE_CONSTRUCTOR) || identical(kind, ElementKind.FUNCTION) || identical(kind, ElementKind.GETTER) || identical(kind, ElementKind.SETTER)) {
  ...
  void resolveRedirectingConstructor(InitializerResolver resolver, Node node, FunctionElement constructor, FunctionElement redirection) {
    assert(invariant(node, constructor.isImplementation, message: 'Redirecting constructors must be resolved on implementation ' 'elements.'));
  ...
  void checkMatchingPatchParameters(FunctionElement origin, Link<Element> originParameters, Link<Element> patchParameters) {
    while (!originParameters.isEmpty) {
      ...
      if (!originParameter.isPatched) {
        ...
        assert(invariant(origin, originParameter.patch == patchParameter, message: "Inconsistent repatch of $originParameter."));
      }
      ...
      if (originParameterType != patchParameterType) {
        compiler.reportError(originParameter.parseNode(compiler), MessageKind.PATCH_PARAMETER_TYPE_MISMATCH, {
          'methodName': origin.name,
          'parameterName': originParameter.name,
          'originParameterType': originParameterType,
          'patchParameterType': patchParameterType
        });
        compiler.reportInfo(patchParameter, MessageKind.PATCH_POINT_TO_PARAMETER, {
        ...
        String originParameterText = originParameter.parseNode(compiler).toString();
        String patchParameterText = patchParameter.parseNode(compiler).toString();
        if (originParameterText != patchParameterText // We special case the list constructor because of the
        // optional parameter.
        && origin != compiler.unnamedListConstructor) {
          compiler.reportError(originParameter.parseNode(compiler), MessageKind.PATCH_PARAMETER_MISMATCH, {
            'methodName': origin.name,
  ...
  void checkMatchingPatchSignatures(FunctionElement origin, FunctionElement patch) {
    ...
    if (originSignature.type.returnType != patchSignature.type.returnType) {
      compiler.withCurrentElement(patch, () {
        Node errorNode = patchTree.returnType != null ? patchTree.returnType : patchTree;
    ...
    if (originSignature.requiredParameterCount != patchSignature.requiredParameterCount) {
      compiler.withCurrentElement(patch, () {
        error(patchTree, MessageKind.PATCH_REQUIRED_PARAMETER_COUNT_MISMATCH, {
          'methodName': origin.name,
      ...
      checkMatchingPatchParameters(origin, originSignature.requiredParameters, patchSignature.requiredParameters);
    }
    if (originSignature.optionalParameterCount != 0 && patchSignature.optionalParameterCount != 0) {
      if (originSignature.optionalParametersAreNamed != patchSignature.optionalParametersAreNamed) {
        compiler.withCurrentElement(patch, () {
          error(patchTree, MessageKind.PATCH_OPTIONAL_PARAMETER_NAMED_MISMATCH, {
            'methodName': origin.name
          });
        });
      }
    }
    if (originSignature.optionalParameterCount != patchSignature.optionalParameterCount) {
      compiler.withCurrentElement(patch, () {
      ...
      checkMatchingPatchParameters(origin, originSignature.optionalParameters, patchSignature.optionalParameters);
    }
  }
  ...
  TreeElements resolveMethodElement(FunctionElementX element) {
    ...
    return compiler.withCurrentElement(element, () {
      bool isConstructor = identical(element.kind, ElementKind.GENERATIVE_CONSTRUCTOR);
      if (compiler.enqueuer.resolution.hasBeenResolved(element)) {
        // TODO(karlklose): Remove the check for [isConstructor]. [elememts]
        // should never be non-null, not even for constructors.
        assert(invariant(element, element.isConstructor, message: 'Non-constructor element $element ' 'has already been analyzed.'));
      ...
      return compiler.withCurrentElement(element, () {
        ...
        if (isConstructor || element.isFactoryConstructor) {
          ...
          if (element.modifiers.isConst && tree.hasBody() && !tree.isRedirectingFactory) {
        ...
        if (enclosingClass != null) {
          ...
          Iterable<MixinApplicationElement> mixinUses = compiler.world.allMixinUsesOf(enclosingClass);
  ...
  ResolverVisitor visitorFor(Element element, {bool useEnclosingScope: false}) {
    return new ResolverVisitor(compiler, element, new ResolutionRegistry(compiler, element), useEnclosingScope: useEnclosingScope);
  }
  ...
  void resolveRedirectionChain(ConstructorElementX constructor, Spannable node) {
    ...
    while (target.isRedirectingFactory) {
      if (target.internalEffectiveTarget != null) {
        ...
        assert(invariant(target, targetType != null, message: 'Redirection target type has not been computed for ' '$target'));
    ...
    while (!seen.isEmpty) {
      ...
      assert(invariant(node, treeElements != null, message: 'No TreeElements cached for $factory.'));
  ...
  void loadSupertypes(BaseClassElementX cls, Spannable from) {
    compiler.withCurrentElement(cls, () => measure(() {
      ...
      if (cls.supertypeLoadState == STATE_STARTED) {
        ...
        cls.allSupertypesAndSelf = compiler.objectClass.allSupertypesAndSelf.extendClass(cls.computeType(compiler));
        cls.supertype = cls.allSupertypes.head;
        assert(invariant(from, cls.supertype != null, message: 'Missing supertype on cyclic class $cls.'));
      ...
      compiler.withCurrentElement(cls, () {
        ...
        cls.parseNode(compiler).accept(new ClassSupertypeResolver(compiler, cls));
  ...
  Queue<ClassElement> pendingClassesToBePostProcessed = new Queue<ClassElement>();
  ...
  _resolveTypeDeclaration(TypeDeclarationElement element, resolveTypeDeclaration()) {
    return compiler.withCurrentElement(element, () {
      return measure(() {
        TypeDeclarationElement previousResolvedTypeDeclaration = currentlyResolvedTypeDeclaration;
        ...
        if (previousResolvedTypeDeclaration == null) {
          do {
            ...
            while (!pendingClassesToBePostProcessed.isEmpty) {
              _postProcessClassElement(pendingClassesToBePostProcessed.removeFirst());
            }
          } while (!pendingClassesToBeResolved.isEmpty);
  ...
  void resolveClassInternal(BaseClassElementX element, ResolutionRegistry registry) {
    if (!element.isPatch) {
      compiler.withCurrentElement(element, () => measure(() {
        ...
        ClassResolverVisitor visitor = new ClassResolverVisitor(compiler, element, registry);
        visitor.visit(tree);
  ...
  void _postProcessClassElement(BaseClassElementX element) {
    for (MetadataAnnotation metadata in element.metadata) {
      metadata.ensureResolved(compiler);
      if (!element.isProxy && metadata.constant.value == compiler.proxyConstant) {
  ...
  void checkMixinApplication(MixinApplicationElementX mixinApplication) {
    ...
    if (illegalFlags != 0) {
      ...
      compiler.reportError(modifiers, MessageKind.ILLEGAL_MIXIN_APPLICATION_MODIFIERS, {
    ...
    if (mixin.superclass == null) {
      compiler.reportError(mixinApplication, MessageKind.ILLEGAL_MIXIN_OBJECT);
    ...
    mixin.forEachLocalMember((AstElement member) {
      if (member.isGenerativeConstructor && !member.isSynthesized) {
        ...
        if (compiler.enqueuer.resolution.hasBeenResolved(member)) {
          checkMixinSuperUses(member.resolvedAst.elements, mixinApplication, mixin);
        }
      }
    });
  }

  void checkMixinSuperUses(TreeElements resolutionTree, MixinApplicationElement mixinApplication, ClassElement mixin) {
    // TODO(johnniwinther): Avoid the use of [TreeElements] here.
    if (resolutionTree == null) return;
    Iterable<Node> superUses = resolutionTree.superUses;
    if (superUses.isEmpty) return;
    compiler.reportError(mixinApplication, MessageKind.ILLEGAL_MIXIN_WITH_SUPER, {
      'className': mixin.name
    });
    // Show the user the problematic uses of 'super' in the mixin.
    for (Node use in superUses) {
      compiler.reportInfo(use, MessageKind.ILLEGAL_MIXIN_SUPER_USE);
    }
  }
  ...
  void checkClassMembers(ClassElement cls) {
    ...
    cls.forEachMember((holder, member) {
      compiler.withCurrentElement(member, () {
        ...
        if (member.isFunction && member.modifiers.isFinal) {
          compiler.reportError(member, MessageKind.ILLEGAL_FINAL_METHOD_MODIFIER);
        }
        if (member.isConstructor) {
          final mismatchedFlagsBits = member.modifiers.flags & (Modifiers.FLAG_STATIC | Modifiers.FLAG_ABSTRACT);
          if (mismatchedFlagsBits != 0) {
            final mismatchedFlags = new Modifiers.withFlags(null, mismatchedFlagsBits);
            compiler.reportError(member, MessageKind.ILLEGAL_CONSTRUCTOR_MODIFIERS, {
        ...
        if (member.isField) {
          if (member.modifiers.isConst && !member.modifiers.isStatic) {
            compiler.reportError(member, MessageKind.ILLEGAL_CONST_FIELD_MODIFIER);
          }
    ...
    if (!constConstructors.isEmpty && !nonFinalInstanceFields.isEmpty) {
      Spannable span = constConstructors.length > 1 ? cls : constConstructors[0];
      compiler.reportError(span, MessageKind.CONST_CONSTRUCTOR_WITH_NONFINAL_FIELDS, {
        'className': cls.name
      });
      if (constConstructors.length > 1) {
        for (Element constructor in constConstructors) {
          compiler.reportInfo(constructor, MessageKind.CONST_CONSTRUCTOR_WITH_NONFINAL_FIELDS_CONSTRUCTOR);
        }
      }
      for (Element field in nonFinalInstanceFields) {
        compiler.reportInfo(field, MessageKind.CONST_CONSTRUCTOR_WITH_NONFINAL_FIELDS_FIELD);
      }
    }
  }
  ...
  void checkAbstractField(Element member) {
    ...
    if (lookupElement == null) {
      compiler.internalError(member, "No abstract field for accessor");
    } else if (!identical(lookupElement.kind, ElementKind.ABSTRACT_FIELD)) {
      compiler.internalError(member, "Inaccessible abstract field for accessor");
    }
    ...
    if (!identical(getterFlags, setterFlags)) {
      final mismatchedFlags = new Modifiers.withFlags(null, getterFlags ^ setterFlags);
  ...
  void checkUserDefinableOperator(Element member) {
    ...
    if (identical(value, 'unary-')) {
      ...
      compiler.internalError(function, 'Unexpected user defined operator $value');
    }
  ...
  void checkOverrideHashCode(FunctionElement operatorEquals) {
    ...
    compiler.reportHint(operatorEquals, MessageKind.OVERRIDE_EQUALS_NOT_HASH_CODE, {
      'class': cls.name
    });
  }

  void checkArity(FunctionElement function, int requiredParameterCount, MessageKind messageKind, bool isMinus) {
    FunctionExpression node = function.node;
    FunctionSignature signature = function.functionSignature;
    if (signature.requiredParameterCount != requiredParameterCount) {
      Node errorNode = node;
      if (node.parameters != null) {
        if (isMinus || signature.requiredParameterCount < requiredParameterCount) {
      ...
      compiler.reportError(errorNode, messageKind, {
        'operatorName': function.name
      });
    }
    if (signature.optionalParameterCount != 0) {
      Node errorNode = node.parameters.nodes.skip(signature.requiredParameterCount).head;
      if (signature.optionalParametersAreNamed) {
        compiler.reportError(errorNode, MessageKind.OPERATOR_NAMED_PARAMETERS, {
          'operatorName': function.name
        });
      } else {
        compiler.reportError(errorNode, MessageKind.OPERATOR_OPTIONAL_PARAMETERS, {
          'operatorName': function.name
        });
      }
    }
  ...
  reportErrorWithContext(Element errorneousElement, MessageKind errorMessage, Element contextElement, MessageKind contextMessage) {
  ...
  FunctionSignature resolveSignature(FunctionElementX element) {
    ...
    return compiler.withCurrentElement(element, () {
      FunctionExpression node = compiler.parser.measure(() => element.parseNode(compiler));
      return measure(() => SignatureResolver.analyze(compiler, node.parameters, node.returnType, element, new ResolutionRegistry(compiler, element), defaultValuesError: defaultValuesError, createRealParameters: true));
    });
  }
    ...
    return _resolveTypeDeclaration(element, () {
      ...
      return compiler.withCurrentElement(element, () {
        return measure(() {
          assert(element.resolutionState == STATE_NOT_STARTED);
          element.resolutionState = STATE_STARTED;
          Typedef node = compiler.parser.measure(() => element.parseNode(compiler));
          TypedefResolverVisitor visitor = new TypedefResolverVisitor(compiler, element, registry);
          visitor.visit(node);
          element.resolutionState = STATE_DONE;
          return registry.mapping;
        });
  ...
  void resolveMetadataAnnotation(MetadataAnnotationX annotation) {
    compiler.withCurrentElement(annotation.annotatedElement, () => measure(() {
      ...
      assert(invariant(node, context != null, message: "No context found for metadata annotation " "on $annotatedElement."));
      ResolverVisitor visitor = visitorFor(context, useEnclosingScope: true);
      ResolutionRegistry registry = visitor.registry;
      node.accept(visitor);
      // TODO(johnniwinther): Avoid passing the [TreeElements] to
      // [compileMetadata].
      annotation.constant = constantCompiler.compileMetadata(annotation, node, registry.mapping);
  ...
  Link<MetadataAnnotation> resolveMetadata(Element element, VariableDefinitions node) {
    LinkBuilder<MetadataAnnotation> metadata = new LinkBuilder<MetadataAnnotation>();
    for (Metadata annotation in node.metadata.nodes) {
      ParameterMetadataAnnotation metadataAnnotation = new ParameterMetadataAnnotation(annotation);
...
class InitializerResolver {
  ...
  InitializerResolver(this.visitor)
      : initialized = new Map<Element, Node>(),
        hasSuper = false;
  ...
  reportDuplicateInitializerError(Element field, Node init, Node existing) {
    visitor.compiler.reportError(init, MessageKind.DUPLICATE_INITIALIZER, {
      'fieldName': field.name
    });
    visitor.compiler.reportInfo(existing, MessageKind.ALREADY_INITIALIZED, {
      'fieldName': field.name
    });
  }
  ...
  void resolveFieldInitializer(FunctionElement constructor, SendSet init) {
    ...
    if (isFieldInitializer(init)) {
      ...
      if (target == null) {
        error(selector, MessageKind.CANNOT_RESOLVE, {
          'name': name
        });
      } else if (target.kind != ElementKind.FIELD) {
        error(selector, MessageKind.NOT_A_FIELD, {
          'fieldName': name
        });
      } else if (!target.isInstanceMember) {
        error(selector, MessageKind.INIT_STATIC_FIELD, {
          'fieldName': name
        });
      }
    } else {
  ...
  ClassElement getSuperOrThisLookupTarget(FunctionElement constructor, bool isSuperCall, Node diagnosticNode) {
  ...
  Element resolveSuperOrThisForSend(FunctionElement constructor, FunctionExpression functionNode, Send call) {
    ...
    ClassElement lookupTarget = getSuperOrThisLookupTarget(constructor, isSuperCall, call);
    Selector constructorSelector = visitor.getRedirectingThisOrSuperConstructorSelector(call);
    FunctionElement calledConstructor = lookupTarget.lookupConstructor(constructorSelector);

    final bool isImplicitSuperCall = false;
    final String className = lookupTarget.name;
    verifyThatConstructorMatchesCall(constructor, calledConstructor, selector, isImplicitSuperCall, call, className, constructorSelector);
  ...
  void resolveImplicitSuperConstructorSend(FunctionElement constructor, FunctionExpression functionNode) {
    ...
    if (classElement != visitor.compiler.objectClass) {
      ...
      Selector callToMatch = new Selector.call(constructorName, classElement.library, 0);

      final bool isSuperCall = true;
      ClassElement lookupTarget = getSuperOrThisLookupTarget(constructor, isSuperCall, functionNode);
      Selector constructorSelector = new Selector.callDefaultConstructor(visitor.enclosingElement.library);
      Element calledConstructor = lookupTarget.lookupConstructor(constructorSelector);

      final String className = lookupTarget.name;
      final bool isImplicitSuperCall = true;
      verifyThatConstructorMatchesCall(constructor, calledConstructor, callToMatch, isImplicitSuperCall, functionNode, className, constructorSelector);
      registry.registerImplicitSuperCall(calledConstructor);
      registry.registerStaticUse(calledConstructor);
    }
  }

  void verifyThatConstructorMatchesCall(FunctionElement caller, FunctionElement lookedupConstructor, Selector call, bool isImplicitSuperCall, Node diagnosticNode, String className, Selector constructorSelector) {
    if (lookedupConstructor == null || !lookedupConstructor.isGenerativeConstructor) {
      String fullConstructorName = Elements.constructorNameForDiagnostics(className, constructorSelector.name);
      MessageKind kind = isImplicitSuperCall ? MessageKind.CANNOT_RESOLVE_CONSTRUCTOR_FOR_IMPLICIT : MessageKind.CANNOT_RESOLVE_CONSTRUCTOR;
      ...
      if (!call.applies(lookedupConstructor, visitor.compiler.world)) {
        MessageKind kind = isImplicitSuperCall ? MessageKind.NO_MATCHING_CONSTRUCTOR_FOR_IMPLICIT : MessageKind.NO_MATCHING_CONSTRUCTOR;
        visitor.compiler.reportError(diagnosticNode, kind);
      } else if (caller.isConst && !lookedupConstructor.isConst) {
        visitor.compiler.reportError(diagnosticNode, MessageKind.CONST_CALLS_NON_CONST);
      }
    }
  }
   ...
   */
  FunctionElement resolveInitializers(FunctionElement constructor, FunctionExpression functionNode) {
    ...
    functionParameters.forEachParameter((ParameterElement element) {
      if (element.isInitializingFormal) {
        ...
        checkForDuplicateInitializers(initializingFormal.fieldElement, element.initializer);
      }
    });
...
class CommonResolverVisitor<R> extends Visitor<R> {
  ...
  R visitNode(Node node) {
    internalError(node, 'internal error: Unhandled node: ${node.getObjectDescription()}');
    return null;
  }
...
class StatementScope {
  ...
  JumpTarget currentBreakTarget() => breakTargetStack.isEmpty ? null : breakTargetStack.head;

  JumpTarget currentContinueTarget() => continueTargetStack.isEmpty ? null : continueTargetStack.head;
  ...
  void enterSwitch(JumpTarget breakElement, Map<String, LabelDefinition> continueElements) {
...
class TypeResolver {
  ...
  Element resolveTypeName(Identifier prefixName, Identifier typeName, Scope scope, {bool deferredIsMalformed: true}) {
    Element element;
    bool deferredTypeAnnotation = false;
    if (prefixName != null) {
      Element prefixElement = lookupInScope(compiler, prefixName, scope, prefixName.source);
      if (prefixElement != null && prefixElement.isPrefix) {
        ...
        if (element != null && prefix.isDeferred && deferredIsMalformed && compiler.backend is! DartBackend) {
  ...
  DartType resolveTypeAnnotation(MappingVisitor visitor, TypeAnnotation node, {bool malformedIsError: false, bool deferredIsMalformed: true}) {
    ResolutionRegistry registry = visitor.registry;
    ...
    DartType checkNoTypeArguments(DartType type) {
      ...
      bool hasTypeArgumentMismatch = resolveTypeArguments(visitor, node, const <DartType>[], arguments);
      if (hasTypeArgumentMismatch) {
        return new MalformedType(new ErroneousElementX(MessageKind.TYPE_ARGUMENT_COUNT_MISMATCH, {
          'type': node
    ...
    Element element = resolveTypeName(prefixName, typeName, visitor.scope, deferredIsMalformed: deferredIsMalformed);

    DartType reportFailureAndCreateType(MessageKind messageKind, Map messageArguments, {DartType userProvidedBadType, Element erroneousElement}) {
      if (malformedIsError) {
      ...
      if (erroneousElement == null) {
        erroneousElement = new ErroneousElementX(messageKind, messageArguments, typeName.source, visitor.enclosingElement);
      }
      List<DartType> arguments = <DartType>[];
      resolveTypeArguments(visitor, node, const <DartType>[], arguments);
      return new MalformedType(erroneousElement, userProvidedBadType, arguments);
    }

    // Try to construct the type from the element.
    if (element == null) {
      type = reportFailureAndCreateType(MessageKind.CANNOT_RESOLVE_TYPE, {
        'typeName': node.typeName
      });
    } else if (element.isAmbiguous) {
      AmbiguousElement ambiguous = element;
      type = reportFailureAndCreateType(ambiguous.messageKind, ambiguous.messageArguments);
      ambiguous.diagnose(registry.mapping.analyzedElement, compiler);
    } else if (element.isErroneous) {
      ErroneousElement erroneousElement = element;
      type = reportFailureAndCreateType(erroneousElement.messageKind, erroneousElement.messageArguments, erroneousElement: erroneousElement);
    } else if (!element.impliesType) {
      type = reportFailureAndCreateType(MessageKind.NOT_A_TYPE, {
        'node': node.typeName
      });
    } else {
      ...
      if (element.isClass) {
        ...
        element.computeType(compiler);
        List<DartType> arguments = <DartType>[];
        bool hasTypeArgumentMismatch = resolveTypeArguments(visitor, node, cls.typeVariables, arguments);
        if (hasTypeArgumentMismatch) {
          type = new BadInterfaceType(cls.declaration, new InterfaceType.forUserProvidedBadType(cls.declaration, arguments));
        } else {
        ...
        element.computeType(compiler);
        List<DartType> arguments = <DartType>[];
        bool hasTypeArgumentMismatch = resolveTypeArguments(visitor, node, typdef.typeVariables, arguments);
        if (hasTypeArgumentMismatch) {
          type = new BadTypedefType(typdef, new TypedefType.forUserProvidedBadType(typdef, arguments));
        } else {
          if (arguments.isEmpty) {
            type = typdef.rawType;
          } else {
        ...
        Element outer = visitor.enclosingElement.outermostEnclosingMemberOrTopLevel;
        bool isInFactoryConstructor = outer != null && outer.isFactoryConstructor;
        if (!outer.isClass && !outer.isTypedef && !isInFactoryConstructor && Elements.isInStaticContext(visitor.enclosingElement)) {
          registry.registerThrowRuntimeError();
          type = reportFailureAndCreateType(MessageKind.TYPE_VARIABLE_WITHIN_STATIC_MEMBER, {
        ...
        compiler.internalError(node, "Unexpected element kind ${element.kind}.");
      }
      if (addTypeVariableBoundsCheck) {
        registry.registerTypeVariableBoundCheck();
        visitor.addDeferredAction(visitor.enclosingElement, () => checkTypeVariableBounds(node, type));
      }
    }
    registry.useType(node, type);
    return type;
  ...
  void checkTypeVariableBounds(TypeAnnotation node, GenericType type) {
    void checkTypeVariableBound(_, DartType typeArgument, TypeVariableType typeVariable, DartType bound) {
      if (!compiler.types.isSubtype(typeArgument, bound)) {
        compiler.reportWarning(node, MessageKind.INVALID_TYPE_VARIABLE_BOUND, {
   ...
   */
  bool resolveTypeArguments(MappingVisitor visitor, TypeAnnotation node, List<DartType> typeVariables, List<DartType> arguments) {
    ...
    for (Link<Node> typeArguments = node.typeArguments.nodes; !typeArguments.isEmpty; typeArguments = typeArguments.tail, index++) {
      if (index > expectedVariables - 1) {
        visitor.warning(typeArguments.head, MessageKind.ADDITIONAL_TYPE_ARGUMENT);
...
abstract class MappingVisitor<T> extends CommonResolverVisitor<T> {
  ...
  void reportDuplicateDefinition(String name, Spannable definition, Spannable existing) {
    compiler.reportError(definition, MessageKind.DUPLICATE_DEFINITION, {
      'name': name
    });
    compiler.reportInfo(existing, MessageKind.EXISTING_DEFINITION, {
      'name': name
    });
  }
}
...
class ResolverVisitor extends MappingVisitor<ResolutionResult> {
  ...
  int allowedCategory = ElementCategory.VARIABLE | ElementCategory.FUNCTION | ElementCategory.IMPLIES_TYPE;
  ...
  bool isPotentiallyMutableTarget(Element target) {
    ...
    return (target.isVariable || target.isParameter) && !(target.isFinal || target.isConst);
  }

  // TODO(ahe): Find a way to share this with runtime implementation.
  static final RegExp symbolValidationPattern = new RegExp(r'^(?:[a-zA-Z$][a-zA-Z$0-9_]*\.)*(?:[a-zA-Z$][a-zA-Z$0-9_]*=?|' r'-|' r'unary-|' r'\[\]=|' r'~|' r'==|' r'\[\]|' r'\*|' r'/|' r'%|' r'~/|' r'\+|' r'<<|' r'>>|' r'>=|' r'>|' r'<=|' r'<|' r'&|' r'\^|' r'\|' r')$');

  ResolverVisitor(Compiler compiler, Element element, ResolutionRegistry registry, {bool useEnclosingScope: false})
      : this.enclosingElement = element,
        // When the element is a field, we are actually resolving its
      // initial value, which should not have access to instance
      // fields.
      inInstanceContext = (element.isInstanceMember && !element.isField) || element.isGenerativeConstructor,
        this.currentClass = element.isClassMember ? element.enclosingClass : null,
        this.statementScope = new StatementScope(),
        scope = useEnclosingScope ? Scope.buildEnclosingScope(element) : element.buildScope(),
        // The type annotations on a typedef do not imply type checks.
      // TODO(karlklose): clean this up (dartbug.com/8870).
      inCheckContext = compiler.enableTypeAssertions && !element.isLibrary && !element.isTypedef && !element.enclosingElement.isTypedef,
        inCatchBlock = false,
  ...
  Element reportLookupErrorIfAny(Element result, Node node, String name) {
    if (!Elements.isUnresolved(result)) {
      if (!inInstanceContext && result.isInstanceMember) {
        compiler.reportError(node, MessageKind.NO_INSTANCE_AVAILABLE, {
          'name': name
        });
        ...
        compiler.reportError(node, ambiguous.messageKind, ambiguous.messageArguments);
        ambiguous.diagnose(enclosingElement, compiler);
        return new ErroneousElementX(ambiguous.messageKind, ambiguous.messageArguments, name, enclosingElement);
      }
    }
    return result;
  }
  ...
  JumpTarget getOrDefineTarget(Node statement) {
    ...
    if (element == null) {
      element = new JumpTargetX(statement, statementScope.nestingLevel, enclosingElement);
  ...
  ErroneousElement warnAndCreateErroneousElement(Node node, String name, MessageKind kind, [Map arguments = const {}]) {
    compiler.reportWarning(node, kind, arguments);
  ...
  ResolutionResult visitIdentifier(Identifier node) {
    if (node.isThis()) {
      if (!inInstanceContext) {
        error(node, MessageKind.NO_INSTANCE_AVAILABLE, {
          'name': node
        });
      }
      return null;
      ...
      if (element == null) {
        if (!inInstanceContext) {
          element = warnAndCreateErroneousElement(node, node.source, MessageKind.CANNOT_RESOLVE, {
            'name': node
          });
  ...
  Selector getRedirectingThisOrSuperConstructorSelector(Send node) {
    if (isNamedConstructor(node)) {
      ...
      return new Selector.callConstructor(constructorName, enclosingElement.library);
    } else {
      return new Selector.callDefaultConstructor(enclosingElement.library);
    }
  }
  ...
  FunctionElement resolveConstructorRedirection(FunctionElementX constructor) {
    ...
    if (!initializers.isEmpty && Initializers.isConstructorRedirect(initializers.head)) {
      Selector selector = getRedirectingThisOrSuperConstructorSelector(initializers.head);
  ...
  void setupFunction(FunctionExpression node, FunctionElement function) {
    ...
    Link<Node> parameterNodes = (node.parameters == null) ? const Link<Node>() : node.parameters.nodes;
    functionParameters.forEachParameter((ParameterElement element) {
  ...
  visitFunctionExpression(FunctionExpression node, {bool inFunctionDeclaration: false}) {
    ...
    LocalFunctionElementX function = new LocalFunctionElementX(name, node, ElementKind.FUNCTION, Modifiers.EMPTY, enclosingElement);
    function.functionSignatureCache = SignatureResolver.analyze(compiler, node.parameters, node.returnType, function, registry, createRealParameters: true);
  ...
  visitIf(If node) {
    ...
    doInPromotionScope(node.thenPart, () => visitIn(node.thenPart, new BlockScope(scope)));
  ...
  ResolutionResult resolveSend(Send node) {
    ...
    if (node.receiver == null) {
      ...
      if (selector.isAssert) {
        if (selector.argumentCount != 1) {
          error(node.selector, MessageKind.WRONG_NUMBER_OF_ARGUMENTS_FOR_ASSERT, {
    ...
    if (identical(name, 'this')) {
      ...
      if (node.isOperator) {
        if (isUserDefinableOperator(name)) {
          ...
          error(node.selector, MessageKind.ILLEGAL_SUPER_SEND, {
            'name': name
          });
        }
      }
      if (!inInstanceContext) {
        error(node.receiver, MessageKind.NO_INSTANCE_AVAILABLE, {
          'name': name
        });
        return null;
      }
      ...
      // super.
      if (target == null) {
        target = warnAndCreateErroneousElement(node, name, MessageKind.NO_SUCH_SUPER_MEMBER, {
    ...
    } else if (resolvedReceiver == null || Elements.isUnresolved(resolvedReceiver.element)) {
      return null;
      ...
      MembersCreator.computeClassMembersByName(compiler, receiverClass.declaration, name);
      ...
      if (target == null || target.isInstanceMember) {
        ...
        // setters.
        MessageKind kind = (target == null) ? MessageKind.MEMBER_NOT_FOUND : MessageKind.MEMBER_NOT_STATIC;
        return new ElementResult(warnAndCreateErroneousElement(node, name, kind, {
          'className': receiverClass.name,
          'memberName': name
        }));
      } else if (isPrivateName(name) && target.library != enclosingElement.library) {
        registry.registerThrowNoSuchMethod();
        return new ElementResult(warnAndCreateErroneousElement(node, name, MessageKind.PRIVATE_ACCESS, {
      ...
      if (Elements.isUnresolved(target)) {
        registry.registerThrowNoSuchMethod();
        return new ElementResult(warnAndCreateErroneousElement(node, name, MessageKind.NO_SUCH_LIBRARY_MEMBER, {
        ...
        target = warnAndCreateErroneousElement(node, name, ambiguous.messageKind, ambiguous.messageArguments);
        ambiguous.diagnose(enclosingElement, compiler);
  ...
  static Selector computeSendSelector(Send node, LibraryElement library, Element element) {
    ...
    if (node.isOperator) {
      ...
      if (identical(string, '!') || identical(string, '&&') || identical(string, '||') || identical(string, 'is') || identical(string, 'as') || identical(string, '?') || identical(string, '>>>')) {
        return null;
      }
      String op = source;
      ...
      if (op == null) {
        ...
        return new Selector.call(source, library, node.argumentsNode.slowLength(), []);
      }
      return node.arguments.isEmpty ? new Selector.unaryOperator(op) : new Selector.binaryOperator(op);
    }
    ...
    for (Link<Node> link = node.argumentsNode.nodes; !link.isEmpty; link = link.tail) {
    ...
    if (element != null && element.isConstructor) {
      return new Selector.callConstructor(element.name, library, arity, named);
    }

    // If we're invoking a closure, we do not have an identifier.
    return (identifier == null) ? new Selector.callClosure(arity, named) : new Selector.call(identifier.source, library, arity, named);
  }
    ...
    for (Link<Node> link = list.nodes; !link.isEmpty; link = link.tail) {
      ...
      if (namedArgument != null) {
        ...
        if (seenNamedArguments.containsKey(source)) {
          reportDuplicateDefinition(source, argument, seenNamedArguments[source]);
        } else {
  ...
  ResolutionResult visitSend(Send node) {
    ...
    if (target != null && target == compiler.mirrorSystemGetNameFunction && !compiler.mirrorUsageAnalyzerTask.hasMirrorUsage(enclosingElement)) {
      compiler.reportHint(node.selector, MessageKind.STATIC_FUNCTION_BLOAT, {
    ...
    if (!Elements.isUnresolved(target)) {
      if (target.isAbstractField) {
        ...
        if (target == null && !inInstanceContext) {
          registry.registerThrowNoSuchMethod();
          target = warnAndCreateErroneousElement(node.selector, field.name, MessageKind.CANNOT_RESOLVE_GETTER);
        }
    ...
    if (node.isOperator) {
      ...
      if (identical(operatorString, 'is')) {
        ...
        DartType type = resolveTypeAnnotation(node.typeAnnotationFromIsCheckOrCast);
        if (type != null) {
          registry.registerIsCheck(type);
        }
        ...
        doInPromotionScope(node.arguments.head, () => resolveArguments(node.argumentsNode));
        resolvedArguments = true;
      }
    }
    ...
    if (node.isCall) {
      if (Elements.isUnresolved(target) || target.isGetter || target.isField || Elements.isClosureSend(node, target)) {
  ...
  ResolutionResult visitSendSet(SendSet node) {
    ...
    if (!(result is AssertResult || Elements.isUnresolved(target))) {
      if (target.isAbstractField) {
        ...
        if (setter == null && !inInstanceContext) {
          setter = warnAndCreateErroneousElement(node.selector, field.name, MessageKind.CANNOT_RESOLVE_SETTER);
          registry.registerThrowNoSuchMethod();
        }
        if (isComplex && getter == null && !inInstanceContext) {
          getter = warnAndCreateErroneousElement(node.selector, field.name, MessageKind.CANNOT_RESOLVE_GETTER);
          registry.registerThrowNoSuchMethod();
        }
      } else if (target.impliesType) {
        setter = warnAndCreateErroneousElement(node.selector, target.name, MessageKind.ASSIGNING_TYPE);
        registry.registerThrowNoSuchMethod();
      } else if (target.isFinal || target.isConst || (target.isFunction && Elements.isStaticOrTopLevelFunction(target) && !target.isSetter)) {
        if (target.isFunction) {
          setter = warnAndCreateErroneousElement(node.selector, target.name, MessageKind.ASSIGNING_METHOD);
        } else {
          setter = warnAndCreateErroneousElement(node.selector, target.name, MessageKind.CANNOT_RESOLVE_SETTER);
        }
        registry.registerThrowNoSuchMethod();
      }
      if (isPotentiallyMutableTarget(target)) {
    ...
    if (isComplex) {
      ...
      if (node.isSuperCall) {
        ...
        if (getter == null) {
          target = warnAndCreateErroneousElement(node, selector.name, MessageKind.NO_SUCH_SUPER_MEMBER, {
            'className': currentClass,
  ...
  visitReturn(Return node) {
    ...
    if (expression != null && enclosingElement.isGenerativeConstructor) {
      // It is a compile-time error if a return statement of the form
      // `return e;` appears in a generative constructor.  (Dart Language
      // Specification 13.12.)
      compiler.reportError(expression, MessageKind.CANNOT_RETURN_FROM_CONSTRUCTOR);
    }
    visit(node.expression);
  }
  ...
  visitRedirectingFactoryBody(RedirectingFactoryBody node) {
    ...
    if (!enclosingElement.isFactoryConstructor) {
      compiler.reportError(node, MessageKind.FACTORY_REDIRECTION_IN_NON_FACTORY);
      compiler.reportHint(enclosingElement, MessageKind.MISSING_FACTORY_KEYWORD);
    }
    ConstructorElementX constructor = enclosingElement;
    bool isConstConstructor = constructor.isConst;
    ConstructorElement redirectionTarget = resolveRedirectingFactory(node, inConstContext: isConstConstructor);
    ...
    FunctionType targetType = redirectionTarget.computeType(compiler).subst(type.typeArguments, targetClass.typeVariables);
    ...
    FunctionSignature targetSignature = redirectionTarget.computeSignature(compiler);
    FunctionSignature constructorSignature = constructor.computeSignature(compiler);
    ...
    registry.registerInstantiatedClass(redirectionTarget.enclosingClass.declaration);
  ...
  visitVariableDefinitions(VariableDefinitions node) {
    ...
    VariableDefinitionsVisitor visitor = new VariableDefinitionsVisitor(compiler, node, this, variables);

    Modifiers modifiers = node.modifiers;
    void reportExtraModifier(String modifier) {
      Node modifierNode;
      for (Link<Node> nodes = modifiers.nodes.nodes; !nodes.isEmpty; nodes = nodes.tail) {
    ...
    if (node.metadata != null) {
      variables.metadata = compiler.resolver.resolveMetadata(enclosingElement, node);
    }
  ...
  ResolutionResult visitNewExpression(NewExpression node) {
    ...
    final bool isMirrorsUsedConstant = node.isConst && (constructor == compiler.mirrorsUsedConstructor);
    ...
    if (node.isConst && type.containsTypeVariables) {
      compiler.reportError(node.send.selector, MessageKind.TYPE_VARIABLE_IN_CONSTANT);
    }
    ...
    if (isSymbolConstructor) {
      if (node.isConst) {
        ...
        ConstantExpression constant = compiler.resolver.constantCompiler.compileNode(argumentNode, registry.mapping);
        ...
        if (!compiler.mirrorUsageAnalyzerTask.hasMirrorUsage(enclosingElement)) {
  ...
  void checkConstMapKeysDontOverrideEquals(Spannable spannable, MapConstantValue map) {
    for (ConstantValue key in map.keys) {
      ...
      if (equals.enclosingClass != compiler.objectClass) {
        compiler.reportError(spannable, MessageKind.CONST_MAP_KEY_OVERRIDES_EQUALS, {
          'type': keyType
        });
      }
    }
  ...
  void analyzeConstant(Node node) {
    addDeferredAction(enclosingElement, () {
      ConstantExpression constant = compiler.resolver.constantCompiler.compileNode(node, registry.mapping);
      ...
      if (argumentsToJsInterceptorConstant != null && argumentsToJsInterceptorConstant.contains(node)) {
        if (value.isType) {
  ...
  ConstructorElement resolveRedirectingFactory(RedirectingFactoryBody node, {bool inConstContext: false}) {
    return node.accept(new ConstructorResolver(compiler, this, inConstContext: inConstContext));
  }

  DartType resolveTypeAnnotation(TypeAnnotation node, {bool malformedIsError: false, bool deferredIsMalformed: true}) {
    DartType type = typeResolver.resolveTypeAnnotation(this, node, malformedIsError: malformedIsError, deferredIsMalformed: deferredIsMalformed);
    if (inCheckContext) {
      registry.registerIsCheck(type);
  ...
  visitLiteralList(LiteralList node) {
    ...
    if (typeArgument != null) {
      if (node.isConst && typeArgument.containsTypeVariables) {
        compiler.reportError(arguments.nodes.head, MessageKind.TYPE_VARIABLE_IN_CONSTANT);
      }
  ...
  visitBreakStatement(BreakStatement node) {
    ...
    if (node.target == null) {
      ...
      if (label == null) {
        error(node.target, MessageKind.UNBOUND_LABEL, {
          'labelName': labelName
        });
        return;
      }
  ...
  visitForIn(ForIn node) {
    ...
    VariableDefinitions variableDefinitions = declaration.asVariableDefinitions();
  ...
  visitLiteralMap(LiteralMap node) {
    ...
    if (valueTypeArgument != null) {
      mapType = new InterfaceType(compiler.mapClass, [keyTypeArgument, valueTypeArgument]);
    } else {
      compiler.mapClass.computeType(compiler);
      mapType = compiler.mapClass.rawType;
    }
    if (node.isConst && mapType.containsTypeVariables) {
      compiler.reportError(arguments, MessageKind.TYPE_VARIABLE_IN_CONSTANT);
    }
  ...
  void checkCaseExpressions(SwitchStatement node) {
    ...
    for (Link<Node> cases = node.cases.nodes; !cases.isEmpty; cases = cases.tail) {
      ...
      for (Node labelOrCase in switchCase.labelsAndCases) {
        ...
        ConstantExpression constant = registry.getConstant(caseMatch.expression);
        ...
        if (firstCaseType == null) {
          ...
          if (caseType.element == compiler.doubleClass) {
            compiler.reportError(node, MessageKind.SWITCH_CASE_VALUE_OVERRIDES_EQUALS, {
              'type': "double"
            });
            ...
            compiler.reportError(firstCase.expression, MessageKind.SWITCH_CASE_VALUE_OVERRIDES_EQUALS, {
              'type': caseType
            });
          }
        } else {
          if (caseType != firstCaseType) {
            if (!hasReportedProblem) {
              compiler.reportError(node, MessageKind.SWITCH_CASE_TYPES_NOT_EQUAL, {
                'type': firstCaseType
              });
              compiler.reportInfo(firstCase.expression, MessageKind.SWITCH_CASE_TYPES_NOT_EQUAL_CASE, {
                'type': firstCaseType
              });
              hasReportedProblem = true;
            }
            compiler.reportInfo(caseMatch.expression, MessageKind.SWITCH_CASE_TYPES_NOT_EQUAL_CASE, {
              'type': caseType
            });
          }
        }
    ...
    while (!cases.isEmpty) {
      ...
      for (Node labelOrCase in switchCase.labelsAndCases) {
        ...
        if (existingElement != null) {
          ...
          compiler.reportError(label, MessageKind.DUPLICATE_LABEL, {
            'labelName': labelName
          });
          compiler.reportInfo(existingElement.label, MessageKind.EXISTING_LABEL, {
            'labelName': labelName
          });
        } else {
          ...
          if (existingElement != null) {
            compiler.reportWarning(label, MessageKind.DUPLICATE_LABEL, {
              'labelName': labelName
            });
            compiler.reportInfo(existingElement.label, MessageKind.EXISTING_LABEL, {
              'labelName': labelName
  ...
  visitCatchBlock(CatchBlock node) {
    ...
    if (node.formals != null) {
      ...
      // modifiers or type.
      for (Link<Node> link = node.formals.nodes; !link.isEmpty; link = link.tail) {
    ...
    if (node.type != null && exceptionDefinition != null) {
      ...
      VariableElementX exceptionElement = registry.getDefinition(exceptionVariable);
    ...
    if (stackTraceDefinition != null) {
      ...
      VariableElementX stackTraceElement = registry.getDefinition(stackTraceVariable);
...
class TypeDefinitionVisitor extends MappingVisitor<DartType> {
  ...
  TypeDefinitionVisitor(Compiler compiler, TypeDeclarationElement element, ResolutionRegistry registry)
      : this.enclosingElement = element,
  ...
  void resolveTypeVariableBounds(NodeList node) {
    ...
    while (!nodeLink.isEmpty) {
      ...
      if (typeNode.bound != null) {
        DartType boundType = typeResolver.resolveTypeAnnotation(this, typeNode.bound);
        variableElement.boundCache = boundType;

        void checkTypeVariableBound() {
          Link<TypeVariableElement> seenTypeVariables = const Link<TypeVariableElement>();
...
class TypedefResolverVisitor extends TypeDefinitionVisitor {
  ...
  TypedefResolverVisitor(Compiler compiler, TypedefElement typedefElement, ResolutionRegistry registry)
  ...
  visitTypedef(Typedef node) {
    ...
    FunctionSignature signature = SignatureResolver.analyze(compiler, node.formals, node.returnType, element, registry, defaultValuesError: MessageKind.TYPEDEF_FORMAL_WITH_DEFAULT);
  ...
  visitTypedefType(TypedefType type, _) {
    ...
    if (seenTypedefs.contains(typedefElement)) {
      if (!hasCyclicReference && identical(element, typedefElement)) {
        ...
        ErroneousElementX erroneousElement = new ErroneousElementX(MessageKind.CYCLIC_TYPEDEF, {
          'typedefName': element.name
        }, element.name, element);
        element.alias = new MalformedType(erroneousElement, typedefElement.alias);
...
class ClassResolverVisitor extends TypeDefinitionVisitor {
  ...
  ClassResolverVisitor(Compiler compiler, ClassElement classElement, ResolutionRegistry registry)
      : super(compiler, classElement, registry);

  DartType visitClassNode(ClassNode node) {
    invariant(node, element != null);
    invariant(element, element.resolutionState == STATE_STARTED, message: () => 'cyclic resolution of class $element');
    ...
    if (element.supertype == null) {
      ...
      if (element != superElement) {
        if (superElement == null) {
          compiler.internalError(node, "Cannot resolve default superclass for $element.");
        } else {
    ...
    if (!element.hasConstructor) {
      ...
      FunctionElement constructor = new SynthesizedConstructorElementX.forDefault(superMember, element);
  ...
  DartType checkMixinType(TypeAnnotation mixinNode) {
    ...
    if (isBlackListed(mixinType)) {
      compiler.reportError(mixinNode, MessageKind.CANNOT_MIXIN, {
        'type': mixinType
      });
  ...
  DartType visitNamedMixinApplication(NamedMixinApplication node) {
    ...
    if (identical(node.classKeyword.stringValue, 'typedef')) {
      ...
      compiler.reportWarning(node.classKeyword, MessageKind.DEPRECATED_TYPEDEF_MIXIN_SYNTAX);
    }
  ...
  DartType applyMixin(DartType supertype, DartType mixinType, Node node) {
    ...
    MixinApplicationElementX mixinApplication = new MixinApplicationElementX("${superName}+${mixinName}", element.compilationUnit, compiler.getNextFreeClassId(), node, new Modifiers.withFlags(new NodeList.empty(), Modifiers.FLAG_ABSTRACT));
    // Create synthetic type variables for the mixin application.
    List<DartType> typeVariables = <DartType>[];
    element.typeVariables.forEach((TypeVariableType type) {
      TypeVariableElementX typeVariableElement = new TypeVariableElementX(type.name, mixinApplication, type.element.node);
    ...
    element.typeVariables.forEach((TypeVariableType type) {
      ...
      typeVariableElement.boundCache = type.element.bound.subst(typeVariables, element.typeVariables);
    });
    ...
    return mixinThisType.subst(element.typeVariables, mixinThisType.typeArguments);
  }

  bool isDefaultConstructor(FunctionElement constructor) {
    return constructor.name == '' && constructor.computeSignature(compiler).parameterCount == 0;
  }

  FunctionElement createForwardingConstructor(ConstructorElement target, ClassElement enclosing) {
    return new SynthesizedConstructorElementX(target.name, target, enclosing, false);
  }

  void doApplyMixinTo(MixinApplicationElementX mixinApplication, DartType supertype, DartType mixinType) {
    ...
    NamedMixinApplication namedMixinApplication = node.asNamedMixinApplication();
    Link<DartType> interfaces = (namedMixinApplication != null) ? resolveInterfaces(namedMixinApplication.interfaces, namedMixinApplication.superclass) : const Link<DartType>();
    ...
    if (mixinApplication.interfaces == null) {
      ...
      assert(invariant(mixinApplication, mixinApplication.hasIncompleteHierarchy));
    }
    ...
    superclass.forEachLocalMember((Element member) {
      ...
      FunctionElement forwarder = createForwardingConstructor(member, mixinApplication);
      if (isPrivateName(member.name) && mixinApplication.library != superclass.library) {
  ...
  InterfaceType resolveMixinFor(MixinApplicationElement mixinApplication, DartType mixinType) {
    ...
    while (current != null && current.isMixinApplication) {
      ...
      if (currentMixinApplication == mixinApplication) {
        compiler.reportError(mixinApplication, MessageKind.ILLEGAL_MIXIN_CYCLE, {
          'mixinName1': current.name,
          'mixinName2': previous.name
        });
  ...
  DartType resolveSupertype(ClassElement cls, TypeAnnotation superclass) {
    ...
    if (supertype != null) {
      if (identical(supertype.kind, TypeKind.MALFORMED_TYPE)) {
        ...
        compiler.reportError(superclass.typeName, MessageKind.CLASS_NAME_EXPECTED);
        return objectType;
    ...
    for (Link<Node> link = interfaces.nodes; !link.isEmpty; link = link.tail) {
      ...
      if (interfaceType != null) {
        if (identical(interfaceType.kind, TypeKind.MALFORMED_TYPE)) {
          compiler.reportError(superclass, MessageKind.CANNOT_IMPLEMENT_MALFORMED, {
            'className': element.name,
          ...
          if (interfaceType == element.supertype) {
            compiler.reportError(superclass, MessageKind.DUPLICATE_EXTENDS_IMPLEMENTS, {
              'type': interfaceType
            });
            compiler.reportError(link.head, MessageKind.DUPLICATE_EXTENDS_IMPLEMENTS, {
              'type': interfaceType
            });
          }
  ...
  void calculateAllSupertypes(BaseClassElementX cls) {
    ...
    if (supertype != null) {
      ...
      for (Link<DartType> interfaces = cls.interfaces; !interfaces.isEmpty; interfaces = interfaces.tail) {
      ...
      cls.allSupertypesAndSelf = new OrderedTypeSet.singleton(cls.computeType(compiler));
    }
  }
   ...
   */
  void addAllSupertypes(OrderedTypeSetBuilder allSupertypes, InterfaceType type) {
    ClassElement classElement = type.element;
    Link<DartType> supertypes = classElement.allSupertypes;
    assert(invariant(element, supertypes != null, message: "Supertypes not computed on $classElement " "during resolution of $element"));
  ...
  isBlackListed(DartType type) {
    ...
    return !identical(lib, compiler.coreLibrary) && !compiler.backend.isBackendLibrary(lib) && (type.isDynamic || identical(type.element, compiler.boolClass) || identical(type.element, compiler.numClass) || identical(type.element, compiler.intClass) || identical(type.element, compiler.doubleClass) || identical(type.element, compiler.stringClass) || identical(type.element, compiler.nullClass));
  }
}
...
class ClassSupertypeResolver extends CommonResolverVisitor {
  ...
  void visitSend(Send node) {
    ...
    if (prefix == null) {
      error(node.receiver, MessageKind.NOT_A_PREFIX, {
        'node': node.receiver
      });
      return;
    }
...
class VariableDefinitionsVisitor extends CommonResolverVisitor<Identifier> {
  ...
  VariableDefinitionsVisitor(Compiler compiler, this.definitions, this.resolver, this.variables)
      : super(compiler) {
  }
  ...
  Identifier visitSendSet(SendSet node) {
    ...
    VariableDefinitionScope scope = new VariableDefinitionScope(resolver.scope, name);
    resolver.visitIn(node.arguments.head, scope);
    if (scope.variableReferencedInInitializer) {
      compiler.reportError(identifier, MessageKind.REFERENCE_IN_INITIALIZATION, {
        'variableName': name
      });
    }
    return identifier;
  ...
  Identifier visitIdentifier(Identifier node) {
    ...
    if (definitions.modifiers.isFinal && !resolver.allowFinalWithoutInitializer) {
  ...
  visitNodeList(NodeList node) {
    for (Link<Node> link = node.nodes; !link.isEmpty; link = link.tail) {
      ...
      LocalVariableElement element = new LocalVariableElementX(name.source, resolver.enclosingElement, variables, name.token);
...
class ConstructorResolver extends CommonResolverVisitor<Element> {
  ...
  ConstructorResolver(Compiler compiler, this.resolver, {bool this.inConstContext: false})
      : super(compiler);
  ...
  failOrReturnErroneousElement(Element enclosing, Node diagnosticNode, String targetName, MessageKind kind, Map arguments) {
  ...
  Selector createConstructorSelector(String constructorName) {
    return constructorName == '' ? new Selector.callDefaultConstructor(resolver.enclosingElement.library) : new Selector.callConstructor(constructorName, resolver.enclosingElement.library);
  }

  FunctionElement resolveConstructor(ClassElement cls, Node diagnosticNode, String constructorName) {
    cls.ensureResolved(compiler);
    Selector selector = createConstructorSelector(constructorName);
    Element result = cls.lookupConstructor(selector);
    if (result == null) {
      String fullConstructorName = Elements.constructorNameForDiagnostics(cls.name, constructorName);
      return failOrReturnErroneousElement(cls, diagnosticNode, fullConstructorName, MessageKind.CANNOT_FIND_CONSTRUCTOR, {
        'constructorName': fullConstructorName
      });
  ...
  Element visitNewExpression(NewExpression node) {
    ...
    assert(invariant(selector, element != null, message: 'No element return for $selector.'));
  ...
  FunctionElement finishConstructorReference(Element element, Node diagnosticNode, Node expression) {
    assert(invariant(diagnosticNode, element != null, message: 'No element return for $diagnosticNode.'));
    ...
    if (!Elements.isUnresolved(element) && !element.isConstructor) {
      if (element.isClass) {
        ...
        element = failOrReturnErroneousElement(element, diagnosticNode, element.name, MessageKind.NOT_A_TYPE, {
          'node': diagnosticNode
        });
      }
    }
  ...
  Element visitTypeAnnotation(TypeAnnotation node) {
    ...
    type = resolver.resolveTypeAnnotation(node, malformedIsError: inConstContext, deferredIsMalformed: false);
  ...
  Element visitSend(Send node) {
    ...
    assert(invariant(node.receiver, element != null, message: 'No element return for $node.receiver.'));
    ...
    if (element.isClass) {
      ...
      if (element == null) {
        return failOrReturnErroneousElement(resolver.enclosingElement, name, name.source, MessageKind.CANNOT_RESOLVE, {
          'name': name
        });
      } else if (!element.isClass) {
        error(node, MessageKind.NOT_A_TYPE, {
          'node': name
        });
      }
    } else {
  ...
  Element visitIdentifier(Identifier node) {
    String name = node.source;
    Element element = resolver.reportLookupErrorIfAny(lookupInScope(compiler, node, resolver.scope, name), node, name);
    registry.useElement(node, element);
    // TODO(johnniwinther): Change errors to warnings, cf. 11.11.1.
    if (element == null) {
      return failOrReturnErroneousElement(resolver.enclosingElement, node, name, MessageKind.CANNOT_RESOLVE, {
        'name': name
      });
    } else if (element.isErroneous) {
      return element;
  ...
  Element visitRedirectingFactoryBody(RedirectingFactoryBody node) {
    ...
    return finishConstructorReference(visit(constructorReference), constructorReference, node);
  }
}

/// Looks up [name] in [scope] and unwraps the result.
Element lookupInScope(Compiler compiler, Node node, Scope scope, String name) {
...
abstract class AnalyzableElementX implements AnalyzableElement {
  ...
  TreeElements get treeElements {
    assert(invariant(this, _treeElements != null, message: "TreeElements have not been computed for $this."));
