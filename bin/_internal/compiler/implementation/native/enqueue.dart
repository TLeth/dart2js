...
abstract class NativeEnqueuerBase implements NativeEnqueuer {
  ...
  final Map<ClassElement, Set<ClassElement>> nonNativeSubclasses = new Map<ClassElement, Set<ClassElement>>();
  ...
  final Map<Node, NativeBehavior> nativeBehaviors = new Map<Node, NativeBehavior>();
  ...
  void processSubclassesOfNativeClasses(Iterable<LibraryElement> libraries) {
    ...
    libraries.forEach((library) {
      library.implementation.forEachLocalMember((element) {
        if (element.isClass) {
          ...
          if (extendsName != null) {
            Set<ClassElement> potentialSubclasses = potentialExtends.putIfAbsent(extendsName, () => new Set<ClassElement>());
    ...
    void walkPotentialSubclasses(ClassElement element) {
      ...
      if (nativeSuperclass != null) {
        ...
        if (!element.isNative) {
          nonNativeSubclasses.putIfAbsent(nativeSuperclass, () => new Set<ClassElement>()).add(element);
        }
  ...
  void findAnnotationClasses() {
    ...
    ClassElement find(name) {
      ...
      if (e == null || e is! ClassElement) {
        compiler.internalError(NO_LOCATION_SPANNABLE, "Could not find implementation class '${name}'.");
      }
      return e;
    }
  ...
  String findJsNameFromAnnotation(Element element) {
    ...
    for (Link<MetadataAnnotation> link = element.metadata; !link.isEmpty; link = link.tail) {
      ...
      if (name == null) {
        ...
        PartialMetadataAnnotation partial = annotation;
        compiler.internalError(annotation, 'Too many JSName annotations: ${partial.parseNode(compiler)}');
      }
    }
    return name;
  }
  ...
  processClass(ClassElementX classElement, cause) {
    ...
    assert(compiler.hasIncrementalSupport || !registeredClasses.contains(classElement));
  ...
  void registerMethodUsed(Element method) {
    processNativeBehavior(NativeBehavior.ofMethod(method, compiler), method);
    flushQueue();
  }

  void registerFieldLoad(Element field) {
    processNativeBehavior(NativeBehavior.ofFieldLoad(field, compiler), field);
    flushQueue();
  }

  void registerFieldStore(Element field) {
    processNativeBehavior(NativeBehavior.ofFieldStore(field, compiler), field);
    flushQueue();
  }
  ...
  void registerJsEmbeddedGlobalCall(Send node, ResolverVisitor resolver) {
    NativeBehavior behavior = NativeBehavior.ofJsEmbeddedGlobalCall(node, compiler, resolver);
    processNativeBehavior(behavior, node);
    nativeBehaviors[node] = behavior;
    flushQueue();
  }
  ...
  processNativeBehavior(NativeBehavior behavior, cause) {
    ...
    for (var type in behavior.typesInstantiated) {
      ...
      if (type is InterfaceType) {
        ...
        } else if (compiler.types.isSubtype(type, backend.listImplementation.rawType)) {
          world.registerInstantiatedClass(type.element, registry);
        }
      }
      assert(type is DartType);
      enqueueUnusedClassesMatching((nativeClass) => compiler.types.isSubtype(nativeClass.thisType, type), cause, 'subtypeof($type)');
    }
  ...
  enqueueUnusedClassesMatching(bool predicate(classElement), cause, [String reason]) {
  ...
  onFirstNativeClass() {
    staticUse(name) {
      backend.enqueue(world, backend.findHelper(name), compiler.globalDependencies);
    }
...
class NativeResolutionEnqueuer extends NativeEnqueuerBase {
    ...
    for (String tag in nativeTagsOfClass(classElement)) {
      ...
      if (owner != null) {
        if (owner != classElement) {
          compiler.internalError(classElement, "Tag '$tag' already in use by '${owner.name}'");
        }
      } else {
  ...
  void logSummary(log(message)) {
    log('Resolved ${registeredClasses.length} native elements used, ' '${unusedClasses.length} native elements dead.');
  }
}
...
class NativeCodegenEnqueuer extends NativeEnqueuerBase {
  ...
  void addSubtypes(ClassElement cls, NativeEmitter emitter) {
    ...
    for (DartType type in cls.allSupertypes) {
      List<Element> subtypes = emitter.subtypes.putIfAbsent(type.element, () => <ClassElement>[]);
      subtypes.add(cls);
    }
    ...
    List<Element> directSubtypes = emitter.directSubtypes.putIfAbsent(superclass, () => <ClassElement>[]);
    directSubtypes.add(cls);
  }

  void logSummary(log(message)) {
    log('Compiled ${registeredClasses.length} native classes, ' '${unusedClasses.length} native classes omitted.');
  }
}
