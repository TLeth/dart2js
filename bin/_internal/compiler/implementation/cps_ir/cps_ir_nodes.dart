...
class InvokeStatic extends Expression implements Invoke {
  ...
  InvokeStatic(this.target, this.selector, Continuation cont, List<Definition> args)
...
class InvokeMethod extends Expression implements Invoke {
  ...
  InvokeMethod(Definition receiver, this.selector, Continuation cont, List<Definition> args)
      : receiver = new Reference(receiver),
        continuation = new Reference(cont),
        arguments = _referenceList(args) {
    assert(selector != null);
    assert(selector.kind == SelectorKind.CALL || selector.kind == SelectorKind.OPERATOR || (selector.kind == SelectorKind.GETTER && arguments.isEmpty) || (selector.kind == SelectorKind.SETTER && arguments.length == 1) || (selector.kind == SelectorKind.INDEX && arguments.length == 1) || (selector.kind == SelectorKind.INDEX && arguments.length == 2));
  }
...
class InvokeConstructor extends Expression implements Invoke {
  ...
  InvokeConstructor(this.type, this.target, this.selector, Continuation cont, List<Definition> args)
      : continuation = new Reference(cont),
        arguments = _referenceList(args) {
    assert(dart2js.invariant(target, target.isErroneous || target.isConstructor, message: "Constructor invocation target is not a constructor: " "$target."));
    assert(dart2js.invariant(target, target.isErroneous || type.isDynamic || type.element == target.enclosingClass.declaration, message: "Constructor invocation type ${type} does not match enclosing " "class of target ${target}."));
  }
...
class SetClosureVariable extends Expression implements InteriorNode {
  ...
  SetClosureVariable(this.variable, Primitive value, {this.isDeclaration: false})
...
class InvokeContinuation extends Expression {
  ...
  InvokeContinuation(Continuation cont, List<Definition> args, {recursive: false})
      : continuation = new Reference(cont),
        arguments = _referenceList(args),
...
class FunctionDefinition extends Node implements InteriorNode {
  ...
  FunctionDefinition(this.element, this.returnContinuation, this.parameters, this.body, this.localConstants, this.defaultParameterValues);

  FunctionDefinition.abstract(this.element, this.parameters, this.defaultParameterValues)
...
abstract class RecursiveVisitor extends Visitor {
  ...
  visitGetClosureVariable(GetClosureVariable node) => processGetClosureVariable(node);
...
class RegisterAllocator extends Visitor {
  ...
  final Map<Element, RegisterArray> elementRegisters = <Element, RegisterArray>{};
