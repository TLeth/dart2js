...
// list.
Set<String> doesNotEscapeListSet = new Set<String>.from(const <String>[// From Object.
  '==', 'hashCode', 'toString', 'noSuchMethod', 'runtimeType', // From Iterable.
  'isEmpty', 'isNotEmpty', 'length', 'any', 'contains', 'every', 'join', // From List.
  'add', 'addAll', 'clear', 'fillRange', 'indexOf', 'insert', 'insertAll', 'lastIndexOf', 'remove', 'removeRange', 'replaceRange', 'setAll', 'setRange', 'shuffle', '[]=', // From JSArray.
  'checkMutable', 'checkGrowable',]);

Set<String> doesNotEscapeMapSet = new Set<String>.from(const <String>[// From Object.
  '==', 'hashCode', 'toString', 'noSuchMethod', 'runtimeType', // from Map.
  'isEmpty', 'isNotEmpty', 'length', 'clear', 'containsKey', 'containsValue', '[]=', // [keys] only allows key values to escape, which we do not track.
  'keys']);

abstract class TracerVisitor<T extends TypeInformation> implements TypeInformationVisitor {
  final T tracedType;
  ...
  void visitElementInContainerTypeInformation(ElementInContainerTypeInformation info) {
    addNewEscapeInformation(info);
  }
  ...
  bool isAddedToContainer(DynamicCallSiteTypeInformation info) {
    ...
    return (selectorName == '[]=' && currentUser == arguments[1]) || (selectorName == 'insert' && currentUser == arguments[1]) || (selectorName == 'add' && currentUser == arguments[0]);
  }
  ...
  void visitDynamicCallSiteTypeInformation(DynamicCallSiteTypeInformation info) {
    if (isAddedToContainer(info)) {
      ...
      if (mask.allocationNode != null) {
        ListTypeInformation list = inferrer.types.allocatedLists[mask.allocationNode];
      ...
      if (mask.allocationNode != null) {
        MapTypeInformation map = inferrer.types.allocatedMaps[mask.allocationNode];
    ...
    if (info.targetsIncludeNoSuchMethod && info.arguments != null && info.arguments.contains(currentUser)) {
  ...
  bool isParameterOfListAddingMethod(Element element) {
    ...
    return (method.name == '[]=') || (method.name == 'add') || (method.name == 'insert');
  }
  ...
  bool isClosure(Element element) {
    ...
    if (element.isInstanceMember && element.name == Compiler.CALL_OPERATOR_NAME) {
      return true;
    }
  ...
  void visitMemberTypeInformation(MemberTypeInformation info) {
    ...
    if (!inferrer.compiler.backend.canBeUsedForGlobalOptimizations(info.element)) {
  ...
  void visitParameterTypeInformation(ParameterTypeInformation info) {
    ...
    if (isParameterOfListAddingMethod(info.element) || isParameterOfMapAddingMethod(info.element)) {
