...
import '../types/types.dart' show TypeMask, FlatTypeMask, UnionTypeMask, TypesInferrer;
...
abstract class ConcreteType {
  ...
  factory ConcreteType.empty(int maxConcreteTypeSize, BaseTypes classBaseTypes) {
    return new UnionType(maxConcreteTypeSize, classBaseTypes, new Set<BaseType>());
  }
   ...
   * type.
   */
  factory ConcreteType.singleton(int maxConcreteTypeSize, BaseTypes classBaseTypes, BaseType baseType) {
...
class UnknownConcreteType implements ConcreteType {
  ...
  Set<BaseType> get baseTypes => new Set<BaseType>.from([const UnknownBaseType()]);
  int get hashCode => 0;
...
class UnionType implements ConcreteType {
  ...
  ConcreteType _simplify(Set<BaseType> baseTypes) {
    ...
    if (baseTypes.contains(classBaseTypes.numBaseType) || (baseTypes.contains(classBaseTypes.intBaseType) && baseTypes.contains(classBaseTypes.doubleBaseType))) {
    ...
    return baseTypes.length > maxConcreteTypeSize ? const UnknownConcreteType() : new UnionType(maxConcreteTypeSize, classBaseTypes, baseTypes);
  }
...
class ConcreteTypeSystem extends TypeSystem<ConcreteType> {
  ...
  ConcreteTypeSystem.internal(ConcreteTypesInferrer inferrer, BaseTypes baseTypes, ConcreteType singleton(BaseType baseType))
      : this.compiler = inferrer.compiler,
        ...
        this.nonNullEmptyType = new ConcreteType.empty(inferrer.compiler.maxConcreteTypeSize, baseTypes)// TODO(polux): have better types here
      ,
  ...
  factory ConcreteTypeSystem(ConcreteTypesInferrer inferrer) {
    ...
    return new ConcreteTypeSystem.internal(inferrer, baseTypes, (BaseType baseType) => new ConcreteType.singleton(compiler.maxConcreteTypeSize, baseTypes, baseType));
  }
  ...
  TypeMask baseTypeToTypeMask(BaseType baseType) {
    if (baseType.isUnknown()) {
      ...
      if (element == compiler.backend.numImplementation) {
        return new TypeMask.nonNullSubclass(compiler.backend.numImplementation, compiler.world);
      } else if (element == compiler.backend.intImplementation) {
        return new TypeMask.nonNullSubclass(compiler.backend.intImplementation, compiler.world);
      } else {
  ...
  @override
  ConcreteType addPhiInput(Local variable, ConcreteType phiType, ConcreteType newType) {
    return computeLUB(phiType, newType);
  }

  @override
  ConcreteType allocateDiamondPhi(ConcreteType firstInput, ConcreteType secondInput) {
  ...
  @override
  ConcreteType narrowType(ConcreteType type, DartType annotation, {bool isNullable: true}) {
  ...
  Selector newTypedSelector(ConcreteType receiver, Selector selector) {
    return new TypedSelector(concreteTypeToTypeMask(receiver), selector, compiler.world);
  }
   ...
   */
  ConcreteType nonNullSubX(ClassElement cls, Iterable<ClassElement> extractor(ClassElement cls)) {
    ...
    void registerClass(ClassElement element) {
      if (!element.isAbstract) {
        result = result.union(new ConcreteType.singleton(compiler.maxConcreteTypeSize, baseTypes, new ClassBaseType(element)));
  ...
  @override
  ConcreteType simplifyPhi(Node node, Local variable, ConcreteType phiType) {
    return phiType;
  }
  ...
  @override
  ConcreteType allocateList(ConcreteType type, Node node, Element enclosing, [ConcreteType elementType, int length]) {
    if (elementType != null) {
  ...
  @override
  ConcreteType allocateMap(ConcreteType type, Node node, Element element, [List<ConcreteType> keyTypes, List<ConcreteType> valueTypes]) {
  ...
  ConcreteType getConcreteTypeFor(TypeMask mask) {
    if (mask.isUnion) {
      ...
      return union.disjointMasks.fold(nonNullEmptyType, (type1, type2) => type1.union(getConcreteTypeFor(type2)));
    } else {
 ...
 */
class ConcreteTypeCartesianProduct extends IterableBase<ConcreteTypesEnvironment> {
  final ConcreteTypesInferrer inferrer;
  final ClassElement typeOfThis;
  final Map<Element, ConcreteType> concreteTypes;
  ConcreteTypeCartesianProduct(this.inferrer, this.typeOfThis, this.concreteTypes);
  Iterator get iterator => concreteTypes.isEmpty ? [new ConcreteTypesEnvironment(typeOfThis)].iterator : new ConcreteTypeCartesianProductIterator(inferrer, typeOfThis, concreteTypes);
 ...
 */
class ConcreteTypeCartesianProductIterator implements Iterator<ConcreteTypesEnvironment> {
  final ConcreteTypesInferrer inferrer;
  ...
  ConcreteTypeCartesianProductIterator(this.inferrer, this.classOfThis, Map<Element, ConcreteType> concreteTypes)
...
class BaseTypes {
  ...
  BaseTypes(Compiler compiler)
      : intBaseType = new ClassBaseType(compiler.backend.intImplementation),
        ...
        growableListBaseType = new ClassBaseType(compiler.backend.growableListImplementation),
        fixedListBaseType = new ClassBaseType(compiler.backend.fixedListImplementation),
        constListBaseType = new ClassBaseType(compiler.backend.constListImplementation),
        mapBaseType = new ClassBaseType(compiler.backend.mapImplementation),
        constMapBaseType = new ClassBaseType(compiler.backend.constMapImplementation),
        objectBaseType = new ClassBaseType(compiler.objectClass),
        typeBaseType = new ClassBaseType(compiler.backend.typeImplementation),
        functionBaseType = new ClassBaseType(compiler.backend.functionImplementation),
        ...
        positiveIntType = new ClassBaseType(compiler.backend.positiveIntImplementation);
}
...
class ConcreteTypesEnvironment {
  ...
  ConcreteTypesEnvironment([this.classOfThis]) : environment = new Map<Element, ConcreteType>();
  ...
  bool operator ==(ConcreteTypesEnvironment other) {
    ...
    for (Element key in environment.keys) {
      if (!other.environment.containsKey(key) || (environment[key] != other.environment[key])) {
        return false;
      }
    }
    return true;
...
class Closures {
  final Compiler compiler;
  final Map<FunctionElement, ClosureEnvironment> closures = new Map<FunctionElement, ClosureEnvironment>();

  Closures(this.compiler);

  /// Returns true if the environment of the closure has changed.
  bool put(FunctionElement closure, ConcreteType typeOfThis, LocalsHandler locals) {
...
class InferenceWorkItem {
  ...
  bool operator ==(other) {
    return (other is InferenceWorkItem) && method == other.method && environment == other.environment;
  }
 ...
 */
class ConcreteTypesInferrer extends InferrerEngine<ConcreteType, ConcreteTypeSystem> implements TypesInferrer {
  ...
  final Map<FunctionElement, Map<ConcreteTypesEnvironment, ConcreteType>> methodToTemplates;
  ...
  ConcreteTypesInferrer(Compiler compiler)
      : methodToTemplates = new Map<FunctionElement, Map<ConcreteTypesEnvironment, ConcreteType>>(),
  ...
  void initialize() {
    ...
    listRemoveLast = jsArrayClass.lookupMember('removeLast');
    List<String> typePreservingOps = const ['+', '-', '*'];
    listConstructor = compiler.listClass.lookupConstructor(new Selector.callConstructor('', compiler.listClass.library)).implementation;
    emptyConcreteType = new ConcreteType.empty(compiler.maxConcreteTypeSize, baseTypes);
  ...
  ConcreteType singletonConcreteType(BaseType baseType) {
    return new ConcreteType.singleton(compiler.maxConcreteTypeSize, baseTypes, baseType);
  }
  ...
  void augmentInferredType(Node node, ConcreteType type) {
    ...
    inferredTypes[node] = (currentType == null) ? type : currentType.union(type);
  }
   ...
   */
  void augmentInferredSelectorType(Selector selector, TypeMask typeOfThis, TypeMask returnType) {
    assert(returnType != null);
    assert(typeOfThis != null);

    selector = selector.asUntyped;
    Map<TypeMask, TypeMask> currentMap = inferredSelectorTypes.putIfAbsent(selector, () => new Map<TypeMask, TypeMask>());
    TypeMask currentReturnType = currentMap[typeOfThis];
    currentMap[typeOfThis] = (currentReturnType == null) ? returnType : typeMaskUnion(currentReturnType, returnType);
  }
  ...
  void augmentParameterType(VariableElement parameter, ConcreteType type) {
    ...
    inferredParameterTypes[parameter] = (oldType == null) ? type : oldType.union(type);
  }
  ...
  void addCaller(FunctionElement callee, Element caller) {
    callers.putIfAbsent(callee, () => new Set<Element>()).add(caller);
  }
  ...
  void addDynamicCaller(Selector callee, FunctionElement caller) {
    dynamicCallers.putIfAbsent(callee.name, () => new Set<FunctionElement>()).add(caller);
  }
  ...
  void addFieldReader(Element field, Element reader) {
    fieldReaders.putIfAbsent(field, () => new Set<Element>()).add(reader);
  }
  ...
  void addCapturedLocalReader(Local local, FunctionElement reader) {
    capturedLocalsReaders.putIfAbsent(local, () => new Set<FunctionElement>()).add(reader);
  }
   ...
   */
  void addClosure(FunctionElement closure, ConcreteType typeOfThis, LocalsHandler locals) {
  ...
  void invalidate(Element methodOrField) {
    if (methodOrField.isField) {
      workQueue.add(new InferenceWorkItem(methodOrField, new ConcreteTypesEnvironment()));
    } else {
      Map<ConcreteTypesEnvironment, ConcreteType> templates = methodToTemplates[methodOrField];
      if (templates != null) {
        templates.forEach((environment, _) {
          workQueue.add(new InferenceWorkItem(methodOrField, environment));
        });
      }
    }
  }
  ...
  Map<ConcreteTypesEnvironment, ConcreteType> getTemplatesOrEmpty(FunctionElement function) {
    return methodToTemplates.putIfAbsent(function, () => new Map<ConcreteTypesEnvironment, ConcreteType>());
  }
  ...
  TypeMask getReturnTypeOfElement(Element element) {
    ...
    Map<ConcreteTypesEnvironment, ConcreteType> templates = methodToTemplates[element];
  ...
  TypeMask getTypeOfSelector(Selector selector) {
    Map<TypeMask, TypeMask> candidates = inferredSelectorTypes[selector.asUntyped];
    if (candidates == null) {
      return null;
    }
    ...
    if (selector.mask == null) {
      ...
      candidates.forEach((TypeMask receiverType, TypeMask returnType) {
        TypeMask intersection = receiverType.intersection(selector.mask, compiler.world);
   ...
   * [receiverType].
   */
  ConcreteType getSendReturnType(Selector selector, FunctionElement function, ClassElement receiverType, ArgumentsTypes<ConcreteType> argumentsTypes) {
    assert(function != null);

    ConcreteType result = emptyConcreteType;
    Map<Element, ConcreteType> argumentMap = associateArguments(function, argumentsTypes);
    ...
    ConcreteTypeCartesianProduct product = new ConcreteTypeCartesianProduct(this, receiverType, argumentMap);
    for (ConcreteTypesEnvironment environment in product) {
      result = result.union(getMonomorphicSendReturnType(function, environment));
    }
    ...
    if (selector != null && receiverType != null) {
      ...
      TypeMask receiverMask = (receiverType == compiler.backend.numImplementation || receiverType == compiler.backend.intImplementation) ? new TypeMask.nonNullSubclass(receiverType.declaration, compiler.world) : new TypeMask.nonNullExact(receiverType.declaration, compiler.world);
      TypeMask resultMask = types.concreteTypeToTypeMask(result);
      augmentInferredSelectorType(selector, receiverMask, resultMask);
    }
   ...
   */
  Map<Element, ConcreteType> associateArguments(FunctionElement function, ArgumentsTypes<ConcreteType> argumentsTypes) {
    ...
    if (signature.optionalParametersAreNamed && argumentsTypes.positional.length > signature.requiredParameterCount) {
      return null;
    }

    handleLeftoverOptionalParameter(ParameterElement parameter) {
      Expression initializer = parameter.initializer;
      result[parameter] = (initializer == null) ? nullConcreteType : analyzeDefaultValue(function, initializer);
    }

    final Iterator<ConcreteType> remainingPositionalArguments = argumentsTypes.positional.iterator;
    // we attach each positional parameter to its corresponding positional
    // argument
    for (Link<Element> requiredParameters = signature.requiredParameters; !requiredParameters.isEmpty; requiredParameters = requiredParameters.tail) {
    ...
    if (signature.optionalParametersAreNamed) {
      ...
      final Map<String, Element> leftOverNamedParameters = new Map<String, Element>();
      for ( ; !remainingOptionalParameters.isEmpty; remainingOptionalParameters = remainingOptionalParameters.tail) {
  ...
  ConcreteType getMonomorphicSendReturnType(FunctionElement function, ConcreteTypesEnvironment environment) {
    Map<ConcreteTypesEnvironment, ConcreteType> template = getTemplatesOrEmpty(function);
   ...
   * special case
   */
  ConcreteType getSpecialCaseReturnType(FunctionElement function, ConcreteTypesEnvironment environment) {
    ...
    if (cls != null) {
      ...
      if ((cls == baseTypes.intBaseType.element || cls == baseTypes.doubleBaseType.element) && (name == '+' || name == '-' || name == '*')) {
  ...
  ConcreteType analyzeMethodOrClosure(Element element, ConcreteTypesEnvironment environment) {
    ...
    return (closureEnv == null) ? analyzeMethod(element, environment) : analyzeClosure(element, closureEnv, environment);
  }

  ConcreteType analyzeMethod(Element element, ConcreteTypesEnvironment environment) {
    TypeInferrerVisitor visitor = new TypeInferrerVisitor(element, this, singletonConcreteType(new ClassBaseType(environment.classOfThis)), environment.environment);
    visitor.run();
    return visitor.returnType;
  }

  ConcreteType analyzeClosure(Element element, ClosureEnvironment closureEnv, ConcreteTypesEnvironment environment) {
    assert(environment.classOfThis == null);
    LocalsHandler locals = (closureEnv.locals != null) ? new LocalsHandler.deepCopyOf(closureEnv.locals) : null;
    TypeInferrerVisitor visitor = new TypeInferrerVisitor(element, this, closureEnv.thisType, environment.environment, locals);
    visitor.run();
    return visitor.returnType;
  }
   ...
   */
  ConcreteType handleSpecialMethod(FunctionElement element, ConcreteTypesEnvironment environment) {
    // We trust the return type of native elements
    if (isNativeElement(element)) {
      var elementType = element.type;
      assert(elementType.isFunctionType);
      return typeOfNativeBehavior(native.NativeBehavior.ofMethod(element, compiler));
    }
    // When List([length]) is called with some length, we must augment
    // listElementType with {null}.
    if (element == listConstructor) {
      Link<Element> parameters = listConstructor.functionSignature.optionalParameters;
  ...
  bool analyzeMain(Element element) {
    initialize();
    workQueue.add(new InferenceWorkItem(element, new ConcreteTypesEnvironment()));
    while (!workQueue.isEmpty) {
      currentWorkItem = workQueue.remove();
      if (currentWorkItem.method.isField) {
        analyzeFieldInitialization(currentWorkItem.method);
      } else {
        Map<ConcreteTypesEnvironment, ConcreteType> template = getTemplatesOrEmpty(currentWorkItem.method);
        template.putIfAbsent(currentWorkItem.environment, () => emptyConcreteType);
        recordReturnType(currentWorkItem.method, analyzeMethodOrClosure(currentWorkItem.method, currentWorkItem.environment));
      }
    }
    return true;
  }
  ...
  @override
  ConcreteType addReturnTypeFor(Element analyzedElement, ConcreteType currentType, ConcreteType newType) {
  ...
  void recordReturnType(Element element, ConcreteType type) {
    ...
    Map<ConcreteTypesEnvironment, ConcreteType> template = getTemplatesOrEmpty(element);
  ...
  @override
  void recordTypeOfFinalField(Node node, Element nodeHolder, Element field, ConcreteType type) {
    augmentFieldType(field, type);
  }

  @override
  void recordTypeOfNonFinalField(Spannable node, Element field, ConcreteType type) {
    augmentFieldType(field, type);
  }
  ...
  Element getRealCaller(Element allegedCaller) {
    ...
    if ((currentMethod != allegedCaller) && currentMethod.isFunction && closures.contains(currentMethod)) {
  ...
  @override
  ConcreteType registerCalledElement(Spannable node, Selector selector, Element caller, Element callee, ArgumentsTypes<ConcreteType> arguments, SideEffects sideEffects, bool inLoop) {
    ...
    if ((selector == null) || (selector.kind == SelectorKind.CALL)) {
      ...
      if (callee.isField) { // toplevel closure call
        ...
        for (FunctionElement function in closures.functionElements) {
          ...
          result = result.union(getSendReturnType(selector, function, null, arguments));
        }
        return result;
      ...
      if (callee.isField) {
        ...
        Element enclosing = callee.enclosingElement.isCompilationUnit ? null : callee.enclosingElement;
        addCaller(callee, caller);
      ...
      if (callee.isField) {
        ...
        Element enclosing = callee.enclosingElement.isCompilationUnit ? null : callee.enclosingElement;
        return getSendReturnType(selector, setter, enclosing, new ArgumentsTypes([argumentType], new Map()));
      }
    } else {
  ...
  @override
  ConcreteType registerCalledSelector(Node node, Selector selector, ConcreteType receiverType, Element caller, ArgumentsTypes<ConcreteType> arguments, SideEffects sideEffects, bool inLoop) {
    caller = getRealCaller(caller);
    switch (selector.kind) {
      case SelectorKind.GETTER:
        return registerDynamicGetterSend(selector, receiverType, caller);
      case SelectorKind.SETTER:
        return registerDynamicSetterSend(selector, receiverType, caller, arguments);
      default:
        return registerDynamicSend(selector, receiverType, caller, arguments);
    }
  }

  ConcreteType registerDynamicGetterSend(Selector selector, ConcreteType receiverType, Element caller) {
    caller = getRealCaller(caller);
    ConcreteType result = emptyConcreteType;
    ...
    void augmentResult(ClassElement baseReceiverType, Element member) {
      if (member.isField) {
        ...
        ArgumentsTypes noArguments = new ArgumentsTypes([], new Map());
        result = result.union(getSendReturnType(selector, member, baseReceiverType, noArguments));
      } else if (member.isFunction) {
        addClosure(member, receiverType, null);
        result = result.union(singletonConcreteType(baseTypes.functionBaseType));
      } else {
    ...
    if (receiverType.isUnknown()) {
      ...
      for (Element member in members) {
        ...
        for (ClassElement cls in getReflexiveSubtypesOf(member.enclosingElement)) {
  ...
  ConcreteType registerDynamicSetterSend(Selector selector, ConcreteType receiverType, Element caller, ArgumentsTypes<ConcreteType> arguments) {
    caller = getRealCaller(caller);
    ConcreteType argumentType = arguments.positional.first;
    ...
    void augmentField(ClassElement receiverType, Element setterOrField) {
      if (setterOrField.isField) {
        ...
        // inferrer.addCaller(setter, currentMethod);
        getSendReturnType(selector, setterOrField, receiverType, new ArgumentsTypes([argumentType], new Map()));
      } else {
        throw new ArgumentError("unexpected element type");
      }
    }
  ...
  ConcreteType registerDynamicSend(Selector selector, ConcreteType receiverType, Element caller, ArgumentsTypes<ConcreteType> arguments) {
    caller = getRealCaller(caller);
    ConcreteType result = emptyConcreteType;
    if (receiverType.isUnknown()) {
      addDynamicCaller(selector, caller);
      ...
      for (Element element in elements) {
        if (element.isFunction) {
          ...
          for (ClassElement cls in getReflexiveSubtypesOf(method.enclosingElement)) {
            result = result.union(getSendReturnType(selector, method, cls, arguments));
          }
        } else { // closure call
  ...
  @override
  void setDefaultTypeOfParameter(ParameterElement parameter, ConcreteType type) {
  ...
  @override
  ConcreteType registerCalledClosure(Node node, Selector selector, ConcreteType closure, Element caller, ArgumentsTypes<ConcreteType> arguments, SideEffects sideEffects, bool inLoop) {
    caller = getRealCaller(caller);
    ConcreteType result = emptyConcreteType;
    for (FunctionElement function in closures.functionElements) {
      addCaller(function, caller);
  ...
  void analyze(Element element, ArgumentsTypes arguments) {
    ...
    getSendReturnType(null, function, currentWorkItem.environment.classOfThis, arguments);
  }
}
...
class TypeInferrerVisitor extends SimpleTypeInferrerVisitor<ConcreteType> {
  ...
  TypeInferrerVisitor(Element element, ConcreteTypesInferrer inferrer, this.thisType, Map<Element, ConcreteType> environment, [LocalsHandler<ConcreteType> handler])
  ...
  ConcreteType visitFunctionExpression(FunctionExpression node) {
    ...
    inferrer.addClosure(element, thisType, new LocalsHandler.deepCopyOf(locals));
  ...
  ConcreteType visitLiteralString(LiteralString node) {
    // TODO(polux): get rid of this hack once we have a natural way of inferring
    // the unknown type.
    if (inferrer.testMode && (node.dartString.slowToString() == "__dynamic_for_test")) {
  ...
  ConcreteType visitLiteralList(LiteralList node) {
    ...
    for (Node element in node.elements.nodes) {
      ...
      elementType = elementType == null ? types.allocatePhi(null, null, type) : types.addPhiInput(null, elementType, type);
      length++;
    }
    elementType = elementType == null ? types.nonNullEmpty() : types.simplifyPhi(null, null, elementType);
    ConcreteType containerType = node.isConst ? types.constListType : types.growableListType;
    return types.allocateList(containerType, node, outermostElement, elementType, length);
  }
