...
/// extension of 'log'.
void enableStatsOutput({CompilerOutputProvider outputProvider, bool xml: true, String name: 'stats', String extension, int examples: 10}) {
  if (_stats != null) {
...
class Stats {
  ...
  /// source.
  void dumpCorrelation(idA, Iterable a, idB, Iterable b, {Map dataA, Map dataB}) {}
}
...
abstract class StatsPrinter {
  ...
  void open(String id, [Map<String, dynamic> data = const <String, dynamic>{}]) {}

  /// Create a stat entry for [id] with additional [data] and content created by
  /// [createChildContent].
  void child(String id, [Map<String, dynamic> data = const <String, dynamic>{}, void createChildContent()]) {
    open(id, data);
...
class _StackTraceNode implements Comparable<_StackTraceNode> {
  ...
  _StackTraceNode.node(List<StackTraceLine> commonPrefix, _StackTraceNode first, _StackTraceNode second)
  ...
  void addSubtrace(StackTraceLines stackTrace) {
    ...
    for (_StackTraceNode subtrace in subtraces) {
      int commonPrefixLength = longestCommonPrefixLength(subtrace.commonPrefix, lines);
      if (commonPrefixLength > 0) {
        ...
        if (commonPrefixLength == subtrace.commonPrefix.length) {
          ...
          subtrace.commonPrefix = subtrace.commonPrefix.sublist(commonPrefixLength);
          subtraces.remove(subtrace);
          subtraces.add(new _StackTraceNode.node(lines.sublist(0, commonPrefixLength), subtrace, new _StackTraceNode.leaf(stackTrace)));
        }
        return;
      }
    }
...
class ActiveStats implements Stats {
  ...
  void recordTrace(key, {int sampleFrequency}) {
    ...
    traceMap.putIfAbsent(key, () => new _StackTraceTree(key, sampleFrequency)).sample();
  ...
  void dumpSets() {
    printer.group('sets', () {
      setsMap.forEach((k, set) {
        dumpIterable('examples', '$k', set.keys, limit: printer.examples, dataMap: set);
      });
    });
  ...
  void dumpCounter(id) {
    ...
    result.forEach((int count, Set examples) {
      ...
      if (examples.length == 1 && (examplesLimit == 0 || !hasData(examples.first))) {
        ...
        printer.child('examples', {
          ...
          examples.forEach((example) {
            dumpIterable('examples', '$example', map[example], limit: examplesLimit, includeCount: false);
          });
        });
      }
      counter++;
  ...
  void dumpCorrelation(keyA, Iterable a, keyB, Iterable b, {Map dataA, Map dataB}) {
    printer.child('correlations', {
      ...
      dumpIterable('correlation', '$keyA && $keyB', aAndB, dataMap: dataA, limit: printer.examples);
      dumpIterable('correlation', '$keyA && !$keyB', aAndNotB, dataMap: dataA, limit: printer.examples);
      dumpIterable('correlation', '!$keyA && $keyB', notAandB, dataMap: dataB, limit: printer.examples);
    });
  }

  void dumpIterable(String tag, String title, Iterable iterable, {int limit, Map dataMap, bool includeCount: true}) {
    if (limit == 0) return;
...
Map<dynamic, Set> inverseMap(Map map, {bool equals(key1, key2), int hashCode(key), bool isValidKey(potentialKey)}) {
  Map<dynamic, Set> result = new LinkedHashMap<dynamic, Set>(equals: equals, hashCode: hashCode, isValidKey: isValidKey);
  map.forEach((k, v) {
