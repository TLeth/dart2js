...
import 'dart2jslib.dart' show Compiler, CompilerTask, DiagnosticListener, MessageKind, Script;
import 'elements/elements.dart';
import 'elements/modelx.dart' show LibraryElementX, MetadataAnnotationX, ClassElementX, FunctionElementX;
...
class PatchParserTask extends CompilerTask {
   ...
   * patches.
   */
  Future patchLibrary(LibraryLoader loader, Uri patchUri, LibraryElement originLibrary) {
    return compiler.readScript(originLibrary, patchUri).then((Script script) {
  ...
  void scanLibraryElements(CompilationUnitElement compilationUnit) {
    measure(() {
      ...
      Listener patchListener = new PatchElementListener(compiler, compilationUnit, idGenerator);
  ...
  void applyContainerPatch(ClassElement originClass, Link<Element> patches) {
...
class PatchElementListener extends ElementListener implements Listener {
  ...
  PatchElementListener(Compiler compiler, CompilationUnitElement patchElement, int idGenerator())
...
void patchElement(Compiler compiler, Element origin, Element patch) {
  if (origin == null) {
    compiler.reportError(patch, MessageKind.PATCH_NON_EXISTING, {
      'name': patch.name
    });
    return;
  }
  if (!(origin.isClass || origin.isConstructor || origin.isFunction || origin.isAbstractField)) {
...
void tryPatchClass(Compiler compiler, Element origin, ClassElement patch) {
  if (!origin.isClass) {
    compiler.reportError(origin, MessageKind.PATCH_NON_CLASS, {
      'className': patch.name
    });
    compiler.reportInfo(patch, MessageKind.PATCH_POINT_TO_CLASS, {
      'className': patch.name
    });
    return;
  }
...
void patchClass(Compiler compiler, ClassElementX origin, ClassElementX patch) {
  if (origin.isPatched) {
    compiler.internalError(origin, "Patching the same class more than once.");
  }
...
checkNativeAnnotation(Compiler compiler, ClassElement cls) {
  EagerAnnotationHandler.checkAnnotation(compiler, cls, const NativeAnnotationHandler());
}
...
abstract class EagerAnnotationHandler {
  ...
  bool apply(Compiler compiler, Element element, MetadataAnnotation annotation);

  /// Checks that the annotation value is valid.
  void validate(Compiler compiler, Element element, MetadataAnnotation annotation, ConstantValue constant);


  /// Checks [element] for metadata matching the [handler]. Return `true` if
  /// matching metadata was found.
  static bool checkAnnotation(Compiler compiler, Element element, EagerAnnotationHandler handler) {
    for (Link<MetadataAnnotation> link = element.metadata; !link.isEmpty; link = link.tail) {
      ...
      if (handler.apply(compiler, element, annotation)) {
        ...
        compiler.enqueuer.resolution.addDeferredAction(element, () {
          ...
          handler.validate(compiler, element, annotation, annotation.constant.value);
        });
        return true;
      }
    }
...
class NativeAnnotationHandler implements EagerAnnotationHandler {
  ...
  String getNativeAnnotation(MetadataAnnotation annotation) {
    if (annotation.beginToken != null && annotation.beginToken.next.value == 'Native') {
      // Skipping '@', 'Native', and '('.
  ...
  void validate(Compiler compiler, Element element, MetadataAnnotation annotation, ConstantValue constant) {
    if (constant.computeType(compiler).element != compiler.nativeAnnotationClass) {
...
class PatchAnnotationHandler implements EagerAnnotationHandler {
  ...
  bool isPatchAnnotation(MetadataAnnotation annotation) {
    return annotation.beginToken != null && annotation.beginToken.next.value == 'patch';
  }
...
void tryPatchGetter(DiagnosticListener listener, Element origin, FunctionElement patch) {
  if (!origin.isAbstractField) {
    listener.reportError(origin, MessageKind.PATCH_NON_GETTER, {
      'name': origin.name
    });
    listener.reportInfo(patch, MessageKind.PATCH_POINT_TO_GETTER, {
  ...
  if (originField.getter == null) {
    listener.reportError(origin, MessageKind.PATCH_NO_GETTER, {
      'getterName': patch.name
    });
    listener.reportInfo(patch, MessageKind.PATCH_POINT_TO_GETTER, {
      'getterName': patch.name
...
void tryPatchSetter(DiagnosticListener listener, Element origin, FunctionElement patch) {
  if (!origin.isAbstractField) {
    listener.reportError(origin, MessageKind.PATCH_NON_SETTER, {
      'name': origin.name
    });
    listener.reportInfo(patch, MessageKind.PATCH_POINT_TO_SETTER, {
  ...
  if (originField.setter == null) {
    listener.reportError(origin, MessageKind.PATCH_NO_SETTER, {
      'setterName': patch.name
    });
    listener.reportInfo(patch, MessageKind.PATCH_POINT_TO_SETTER, {
      'setterName': patch.name
...
void tryPatchConstructor(DiagnosticListener listener, Element origin, FunctionElement patch) {
...
void tryPatchFunction(DiagnosticListener listener, Element origin, FunctionElement patch) {
  if (!origin.isFunction) {
    listener.reportError(origin, MessageKind.PATCH_NON_FUNCTION, {
...
void patchFunction(DiagnosticListener listener, FunctionElementX origin, FunctionElementX patch) {
  ...
  if (origin.isPatched) {
    listener.internalError(origin, "Trying to patch a function more than once.");
  }
  origin.applyPatch(patch);
}
...
bool isPatchElement(Compiler compiler, Element element) {
  return EagerAnnotationHandler.checkAnnotation(compiler, element, const PatchAnnotationHandler());
}
