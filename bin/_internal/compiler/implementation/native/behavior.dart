...
class NativeBehavior {
  ...
  static void processSpecString(DiagnosticListener listener, Spannable spannable, String specString, {dynamic resolveType(String typeString), List typesReturned, List typesInstantiated, objectType, nullType}) {
    ...
    void resolveTypesString(String typesString, {onVoid(), onVar(), onType(type)}) {
    ...
    if (specString.contains(':')) {
      ...
      String getTypesString(String tag) {
        ...
        String typeString = specString.substring(startPos + marker.length, endPos);
        specString = '${specString.substring(0, startPos)}' '${specString.substring(endPos + 1)}'.trim();
        return typeString;
      }
      ...
      if (creates != null) {
        resolveTypesString(creates, onVoid: () {
          listener.internalError(spannable, "Invalid type string 'creates:$creates'");
        }, onVar: () {
          listener.internalError(spannable, "Invalid type string 'creates:$creates'");
        }, onType: (type) {
  ...
  static NativeBehavior ofJsCall(Send jsCall, Compiler compiler, resolver) {
    ...
    resolveType(String typeString) {
      return _parseType(typeString, compiler, (name) => resolver.resolveTypeFromString(specLiteral, name), jsCall);
    }

    processSpecString(compiler, jsCall, specString, resolveType: resolveType, typesReturned: behavior.typesReturned, typesInstantiated: behavior.typesInstantiated, objectType: compiler.objectClass.computeType(compiler), nullType: compiler.nullClass.computeType(compiler));

    return behavior;
  }

  static NativeBehavior ofJsEmbeddedGlobalCall(Send jsGlobalCall, Compiler compiler, resolver) {
    ...
    if (argNodes.isEmpty) {
      compiler.internalError(jsGlobalCall, "JS embedded global expression has no type.");
    }
    ...
    if (!argNodes.tail.tail.isEmpty) {
      compiler.internalError(argNodes.tail.tail.head, 'Embedded Global has more than 2 arguments');
    }
    ...
    resolveType(String typeString) {
      return _parseType(typeString, compiler, (name) => resolver.resolveTypeFromString(specLiteral, name), jsGlobalCall);
    }

    processSpecString(compiler, jsGlobalCall, specString, resolveType: resolveType, typesReturned: behavior.typesReturned, typesInstantiated: behavior.typesInstantiated, objectType: compiler.objectClass.computeType(compiler), nullType: compiler.nullClass.computeType(compiler));
  ...
  static NativeBehavior ofMethod(FunctionElement method, Compiler compiler) {
    ...
    // removed.
    method.functionSignature.forEachOptionalParameter((ParameterElement parameter) {
  ...
  void _overrideWithAnnotations(Element element, Compiler compiler) {
    ...
    var creates = _collect(element, compiler, enqueuer.annotationCreatesClass, lookup);
    var returns = _collect(element, compiler, enqueuer.annotationReturnsClass, lookup);
   ...
   */
  static _collect(Element element, Compiler compiler, Element annotationClass, lookup(str)) {
    var types = null;
    for (Link<MetadataAnnotation> link = element.metadata; !link.isEmpty; link = link.tail) {
      ...
      if (fields.length != 1 || !fields[0].isString) {
        ...
        compiler.internalError(annotation, 'Annotations needs one string: ${partial.parseNode(compiler)}');
      }
  ...
  static _parseType(String typeString, Compiler compiler, lookup(name), locationNodeOrElement) {
    ...
    if (index < 1) {
      compiler.internalError(_errorNode(locationNodeOrElement, compiler), "Type '$typeString' not found.");
    }
