...
class JsBuilder {
  ...
  Template uncachedExpressionTemplate(String source) {
    MiniJsParser parser = new MiniJsParser(source);
    Expression expression = parser.expression();
    return new Template(source, expression, isExpression: true, forceCopy: false);
  }
  ...
  Template uncachedStatementTemplate(String source) {
    MiniJsParser parser = new MiniJsParser(source);
    Statement statement = parser.statement();
    return new Template(source, statement, isExpression: false, forceCopy: false);
  }
  ...
  ArrayInitializer numArray(Iterable<int> list) => new ArrayInitializer.from(list.map(number));

  ArrayInitializer stringArray(Iterable<String> list) => new ArrayInitializer.from(list.map(string));
...
class MiniJsParser {
  ...
  static const CATEGORIES = const <int>[OTHER, OTHER, OTHER, OTHER, OTHER, OTHER, OTHER, OTHER, // 0-7
  ...
  static final UNARY_OPERATORS = ['++', '--', '+', '-', '~', '!', 'typeof', 'void', 'delete'].toSet();

  static final OPERATORS_THAT_LOOK_LIKE_IDENTIFIERS = ['typeof', 'void', 'delete', 'in', 'instanceof'].toSet();
  ...
  String getDelimited(int startPosition) {
    ...
    do {
      ...
      if (currentCode == charCodes.$BACKSLASH) {
        ...
        if (escaped == charCodes.$x || escaped == charCodes.$X || escaped == charCodes.$u || escaped == charCodes.$U || category(escaped) == NUMERIC) {
  ...
  void getToken() {
    ...
    if (code == charCodes.$SQ || code == charCodes.$DQ) {
    ...
    } else if (code == charCodes.$0 && position + 2 < src.length && src.codeUnitAt(position + 1) == charCodes.$x) {
      // Hex literal.
      ...
      do {
        ...
        newCat = (code == charCodes.$BANG || code == charCodes.$SLASH) ? NONE : category(code);
      } while (!singleCharCategory(cat) && (cat == newCat || (cat == ALPHA && newCat == NUMERIC) || // eg. level42.
  ...
  Expression parsePrimary() {
    ...
    if (acceptCategory(ALPHA)) {
      ...
      InterpolatedExpression expression = new InterpolatedExpression(interpolatedValues.length);
  ...
  Expression parseFunctionExpression() {
    ...
    if (acceptCategory(ALPHA)) {
      ...
      return new NamedFunction(new VariableDeclaration(functionName), parseFun());
    }
    return parseFun();
  }
    ...
    if (!acceptCategory(RPAREN)) {
      for ( ; ; ) {
        if (acceptCategory(HASH)) {
          InterpolatedParameter parameter = new InterpolatedParameter(interpolatedValues.length);
  ...
  Expression parseObjectInitializer() {
    ...
    for ( ; ; ) {
      ...
      if (acceptCategory(ALPHA)) {
        ...
        InterpolatedLiteral interpolatedLiteral = new InterpolatedLiteral(interpolatedValues.length);
  ...
  Expression parseCall() {
    ...
    while (true) {
      if (acceptCategory(LPAREN)) {
        ...
        receiver = constructor ? new New(receiver, arguments) : new Call(receiver, arguments);
        constructor = false;
  ...
  Expression getDotRhs(Expression receiver) {
    if (acceptCategory(HASH)) {
      InterpolatedSelector property = new InterpolatedSelector(interpolatedValues.length);
  ...
  Expression parsePostfix() {
    ...
    if (lastCategory == SYMBOL && !skippedNewline && (acceptString("++") || acceptString("--"))) {
  ...
  Expression parseUnaryHigh() {
    String operator = lastToken;
    if (lastCategory == SYMBOL && UNARY_OPERATORS.contains(operator) && (acceptString("++") || acceptString("--"))) {
  ...
  Expression parseUnaryLow() {
    String operator = lastToken;
    if (lastCategory == SYMBOL && UNARY_OPERATORS.contains(operator) && operator != "++" && operator != "--") {
  ...
  Expression parseBinary(int maxPrecedence) {
    ...
    while (true) {
      ...
      if (lastCategory != SYMBOL || !BINARY_PRECEDENCE.containsKey(symbol) || BINARY_PRECEDENCE[symbol] > maxPrecedence) {
        break;
      }
      expectCategory(SYMBOL);
  ...
  Expression parseAssignment() {
    ...
    if (acceptCategory(ASSIGNMENT)) {
      ...
      if (assignmentOperator == "=") {
        ...
        // Handle +=, -=, etc.
        String operator = assignmentOperator.substring(0, assignmentOperator.length - 1);
  ...
  Statement parseStatement() {
    ...
    if (lastCategory == ALPHA) {
      ...
      if (lastToken == 'case' || lastToken == 'do' || lastToken == 'while' || lastToken == 'switch' || lastToken == 'with') {
    ...
    if (checkForInterpolatedStatement) {
      ...
      if (expression is InterpolatedExpression) {
        ...
        InterpolatedStatement statement = new InterpolatedStatement(expression.name);
  ...
  Statement parseFor() {
    ...
    if (acceptString('var')) {
      ...
      if (acceptString('in')) {
        ...
        expectCategory(RPAREN);
        Statement body = parseStatement();
        return new ForIn(new VariableDeclarationList([new VariableInitialization(new VariableDeclaration(identifier), null)]), objectExpression, body);
      }
