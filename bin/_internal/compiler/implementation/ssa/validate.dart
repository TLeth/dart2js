...
class HValidator extends HInstructionVisitor {
  ...
  void visitBasicBlock(HBasicBlock block) {
    ...
    if ((block.last is HReturn || block.last is HThrow) && (block.successors.length != 1 || !block.successors[0].isExitBlock())) {
      markInvalid("Return or throw node with > 1 successor " "or not going to exit-block");
    }
    ...
    if (isValid && block.successors.length > 1 && block.last is! HTry && block.last is! HExitTry && block.last is! HSwitch) {
      for (HBasicBlock successor in block.successors) {
        if (!isValid) break;
  ...
  static int countInstruction(List<HInstruction> instructions, HInstruction instruction) {
    int result = 0;
  ...
  void visitInstruction(HInstruction instruction) {
    ...
    bool hasCorrectInputs() {
      ...
      return everyInstruction(instruction.inputs, (input, count) {
        if (inBasicBlock) {
          return input.isInBasicBlock() && countInstruction(input.usedBy, instruction) == count;
        } else {
    ...
    bool hasCorrectUses() {
      ...
      return everyInstruction(instruction.usedBy, (use, count) {
        return use.isInBasicBlock() && countInstruction(use.inputs, instruction) == count;
      });
    }
