...
abstract class SourceFileProvider {
  ...
  Future<List<int>> _readFromFile(Uri resourceUri) {
    ...
    try {
      ...
      return new Future.error("Error reading '${relativize(cwd, resourceUri, isWindows)}' " "(${ex.osError})");
    }
    dartCharactersRead += source.length;
    sourceFiles[resourceUri.toString()] = new CachingUtf8BytesSourceFile(relativizeUri(resourceUri), source);
  ...
  Future<List<int>> _readFromHttp(Uri resourceUri) {
    ...
    return client.getUrl(resourceUri).then((HttpClientRequest request) => request.close()).then((HttpClientResponse response) {
      if (response.statusCode != HttpStatus.OK) {
        String msg = 'Failure getting $resourceUri: ' '${response.statusCode} ${response.reasonPhrase}';
        throw msg;
      }
      ...
      sourceFiles[resourceUri.toString()] = new CachingUtf8BytesSourceFile(resourceUri.toString(), result);
      return result;
    });
  }
...
class FormattingDiagnosticHandler {
  ...
  final int INFO = api.Diagnostic.INFO.ordinal | api.Diagnostic.VERBOSE_INFO.ordinal;

  FormattingDiagnosticHandler([SourceFileProvider provider])
      : this.provider = (provider == null) ? new CompilerSourceFileProvider() : provider;
  ...
  void diagnosticHandler(Uri uri, int begin, int end, String message, api.Diagnostic kind) {
    ...
    if (uri == null) {
      ...
      if (file != null) {
        print(file.getLocationMessage(color(message), begin, end, colorize: color));
      } else {
        print('${provider.relativizeUri(uri)}@$begin+${end - begin}:' ' [$kind] ${color(message)}');
      }
    }
...
class RandomAccessFileOutputProvider {
  ...
  RandomAccessFileOutputProvider(this.out, this.sourceMapOut, {this.onInfo, this.onFailure});
  ...
  EventSink<String> call(String name, String extension) {
    ...
    if (name == '') {
      if (extension == 'js' || extension == 'dart') {
        ...
        uri = out;
        sourceMapFileName = sourceMapOut.path.substring(sourceMapOut.path.lastIndexOf('/') + 1);
      } else if (extension == 'precompiled.js') {
        uri = computePrecompiledUri(out);
        onInfo("File ($uri) is compatible with header" " \"Content-Security-Policy: script-src 'self'\"");
    ...
    writeStringSync(String data) {
      ...
      while (offset < data.length) {
        output.writeStringSync(data.substring(offset, math.min(offset + chunkSize, data.length)));
        offset += chunkSize;
      }
