...
class ContainerBuilder extends CodeEmitterHelper {
  ...
  bool needsSuperGetter(FunctionElement element) => compiler.codegenWorld.methodsNeedingSuperGetter.contains(element);
   ...
   */
  void addParameterStub(FunctionElement member, Selector selector, AddStubFunction addStub, Set<String> alreadyGenerated) {
    ...
    if (parameters.optionalParametersAreNamed && selector.namedArgumentCount == parameters.optionalParameterCount) {
    ...
    List<jsAst.Parameter> parametersBuffer = new List<jsAst.Parameter>(selector.argumentCount + extraArgumentCount);
    // The arguments that will be passed to the real method.
    List<jsAst.Expression> argumentsBuffer = new List<jsAst.Expression>(parameters.parameterCount + extraArgumentCount);
    ...
    parameters.orderedForEachParameter((ParameterElement element) {
      ...
      if (count < optionalParameterStart) {
        ...
        if (index != -1) {
          ...
          argumentsBuffer[count] = js('#', jsName);
          parametersBuffer[optionalParameterStart + index] = new jsAst.Parameter(jsName);
        } else {
          ConstantExpression constant = handler.getConstantForVariable(element);
          if (constant == null) {
            argumentsBuffer[count] = emitter.constantReference(new NullConstantValue());
          } else {
    ...
    if (member.hasFixedBackendName) {
      body = emitter.nativeEmitter.generateParameterStubStatements(member, isInterceptedMethod, invocationName, parametersBuffer, argumentsBuffer, indexOfLastOptionalArgumentInParameters);
      ...
      if (needsSuperGetter(member)) {
        ...
        body = js.statement('return #.prototype.#.call(this, #);', [backend.namer.elementAccess(superClass), methodName, argumentsBuffer]);
      } else {
        body = js.statement('return this.#(#);', [namer.getNameOfInstanceMember(member), argumentsBuffer]);
      }
    } else {
      body = js.statement('return #(#)', [namer.elementAccess(member), argumentsBuffer]);
    }
  ...
  void addParameterStubs(FunctionElement member, AddStubFunction defineStub, [bool canTearOff = false]) {
    ...
    Set<Selector> selectors = member.isInstanceMember ? compiler.codegenWorld.invokedNames[member.name] : null; // No stubs needed for static methods.
    ...
    Set<Selector> callSelectorsAsNamed() {
      ...
      Set<Selector> callSelectors = compiler.codegenWorld.invokedNames[namer.closureInvocationSelectorName];
      if (callSelectors == null) return null;
      return callSelectors.map((Selector callSelector) {
        return new Selector.call(member.name, member.library, callSelector.argumentCount, callSelector.namedArguments);
      }).toSet();
    }
    if (selectors == null) {
    ...
    if (canTearOff) {
      selectors = compiler.codegenWorld.invokedNames[namer.closureInvocationSelectorName];
      if (selectors != null) {
        for (Selector selector in selectors) {
          selector = new Selector.call(member.name, member.library, selector.argumentCount, selector.namedArguments);
          if (!selector.appliesUnnamed(member, compiler.world)) continue;
   ...
   */
  void emitCallStubForGetter(Element member, Set<Selector> selectors, AddPropertyFunction addProperty) {
    ...
    jsAst.Expression buildGetter() {
      jsAst.Expression receiver = js(isInterceptorClass ? receiverArgumentName : 'this');
      if (member.isGetter) {
    ...
    for (Selector selector in selectors) {
      if (selector.applies(member, compiler.world)) {
        ...
        jsAst.Fun function = js('function(#) { return #.#(#); }', [parameters, buildGetter(), closureCallName, arguments]);

        compiler.dumpInfoTask.registerElementAst(member, addProperty(invocationName, function));
      }
    }
  }
  ...
  void addMember(Element member, ClassBuilder builder) {
    ...
    } else if (member.isFunction || member.isGenerativeConstructorBody || member.isGenerativeConstructor || member.isAccessor) {
      addMemberMethod(member, builder);
    } else {
      compiler.internalError(member, 'Unexpected kind: "${member.kind}".');
    }
  ...
  MemberInfo analyzeMemberMethod(FunctionElement member) {
    ...
    bool isNotApplyTarget = !member.isFunction || member.isConstructor || member.isAccessor;
    String tearOffName;
    ...
    if (isNotApplyTarget) {
      ...
      if (member.enclosingClass.isClosure) {
        ...
        canTearOff = compiler.codegenWorld.hasInvokedGetter(member, compiler.world) || (canBeReflected && !member.isOperator);
        assert(!needsSuperGetter(member) || canTearOff);
        tearOffName = namer.getterName(member);
      }
    } else {
      canTearOff = compiler.codegenWorld.staticFunctionsNeedingGetter.contains(member) || canBeReflected;
      tearOffName = namer.getStaticClosureName(member);
    }
    final bool canBeApplied = compiler.enabledFunctionApply && compiler.world.getMightBePassedToApply(member);

    final bool needStructuredInfo = canTearOff || canBeReflected || canBeApplied;
    ...
    return new MemberInfo(member, name, parameters, code, needsStubs: needsStubs, canTearOff: canTearOff, isClosure: isClosure, tearOffName: tearOffName, canBeReflected: canBeReflected, canBeApplied: canBeApplied, needStructuredInfo: needStructuredInfo);
  ...
  void addMemberMethodFromInfo(MemberInfo info, ClassBuilder builder) {
    ...
    if (!needStructuredInfo) {
      compiler.dumpInfoTask.registerElementAst(member, builder.addProperty(name, code));
      if (needsStubs) {
        addParameterStubs(member, (Selector selector, jsAst.Fun function) {
          compiler.dumpInfoTask.registerElementAst(member, builder.addProperty(namer.invocationName(selector), function));
        });
      }
      return;
    }
    ...
    if (needsStubs || canTearOff) {
      addParameterStubs(member, (Selector selector, jsAst.Fun function) {
        ...
        if (member.isInstanceMember) {
          Set invokedSelectors = compiler.codegenWorld.invokedNames[member.name];
    ...
    if (canTearOff || canBeReflected) {
      ...
      if (memberType.containsTypeVariables) {
        ...
        memberTypeExpression = backend.rti.getSignatureEncoding(memberType, thisAccess);
      } else {
        memberTypeExpression = js.number(emitter.metadataEmitter.reifyType(memberType));
      }
    } else {
    ...
    expressions
        ..addAll(tearOffInfo)
        ..add((tearOffName == null || member.isAccessor) ? js("null") : js.string(tearOffName))
        ..add(js.number(requiredParameterCount))
        ..add(js.number(optionalParameterCount))
        ..add(memberTypeExpression)
        ..addAll(emitter.metadataEmitter.reifyDefaultArguments(member).map(js.number));

    if (canBeReflected || canBeApplied) {
      parameters.forEachParameter((Element parameter) {
        expressions.add(js.number(emitter.metadataEmitter.reifyName(parameter.name)));
        if (backend.mustRetainMetadata) {
          Iterable<int> metadataIndices = parameter.metadata.map((MetadataAnnotation annotation) {
            ConstantValue constant = backend.constants.getConstantForMetadata(annotation).value;
            backend.constants.addCompileTimeConstantForEmission(constant);
            return emitter.metadataEmitter.reifyMetadata(annotation);
          });
          expressions.add(new jsAst.ArrayInitializer.from(metadataIndices.map(js.number)));
        }
      });
    }
    if (canBeReflected) {
      ...
      if (member.isConstructor) {
        ...
        reflectionName = new jsAst.LiteralString('"new ${Elements.reconstructConstructorName(member)}"');
      } else {
        reflectionName = js.string(namer.privateName(member.library, member.name));
      }
      expressions
          ..add(reflectionName)
          ..addAll(emitter.metadataEmitter.computeMetadata(member).map(js.number));
    } else if (isClosure && canBeApplied) {
      expressions.add(js.string(namer.privateName(member.library, member.name)));
    }
    jsAst.ArrayInitializer arrayInit = new jsAst.ArrayInitializer.from(expressions);
    compiler.dumpInfoTask.registerElementAst(member, builder.addProperty(name, arrayInit));
  }
...
class MemberInfo {
  ...
  MemberInfo(this.member, this.name, this.parameters, this.code, {this.needsStubs, this.canTearOff, this.isClosure, this.tearOffName, this.canBeReflected, this.canBeApplied, this.needStructuredInfo}) {
    assert(member != null);
    assert(name != null);
