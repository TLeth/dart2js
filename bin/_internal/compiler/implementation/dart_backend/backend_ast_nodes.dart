...
class For extends Statement {
  ...
  For(this.initializer, this.condition, this.updates, this.body) {
    assert(initializer == null || initializer is VariableDeclarations || initializer is Expression);
  }
}
...
class ForIn extends Statement {
  ...
  ForIn(Node leftHandValue, this.expression, this.body)
    ...
    assert(leftHandValue is Identifier || (leftHandValue is VariableDeclarations && leftHandValue.declarations.length == 1 && leftHandValue.declarations[0].initializer == null));
  }
}
...
class VariableDeclarations extends Statement {
  ...
  VariableDeclarations(this.declarations, {this.type, this.isFinal: false, this.isConst: false}) {
...
class Parameters extends Node {
  ...
  Parameters(this.requiredParameters, [this.optionalParameters, this.hasNamedParameters = false]);
  ...
  bool get hasOptionalParameters => optionalParameters != null && optionalParameters.length > 0;
}
...
class Parameter extends Node {
  ...
  Parameter.function(this.name, TypeAnnotation returnType, this.parameters, [this.defaultValue]) : type = returnType {
...
class FunctionExpression extends Expression {
  ...
  FunctionExpression(this.parameters, this.body, {this.name, this.returnType, this.isGetter: false, this.isSetter: false}) {
...
class LiteralMap extends Expression {
  ...
  LiteralMap(this.entries, {this.typeArguments, this.isConst: false}) {
    assert(this.typeArguments == null || this.typeArguments.length == 0 || this.typeArguments.length == 2);
  }
}
...
class CallNew extends Expression {
  ...
  CallNew(this.type, this.arguments, {this.constructorName, this.isConst: false});
}
...
bool isIdentifierPartNoDollar(dynamic x) {
  ...
  return (characters.$0 <= x && x <= characters.$9) || (characters.$A <= x && x <= characters.$Z) || (characters.$a <= x && x <= characters.$z) || (x == characters.$_);
}
...
class Unparser {
  ...
  Statement unfoldBlocks(Statement stmt) {
    ...
    if (stmt is Block && stmt.statements.length == 0) return new EmptyStatement();
    return stmt;
  }
  ...
  void writeExp(Receiver e, int minPrecedence, {beginStmt: false}) {
    ...
    if (e is SuperReceiver) {
      ...
      // !(x == y) ==> x != y.
      if (e.operatorName == '!' && operand is BinaryOperator && operand.operator == '==') {
      ...
      else if (e.operatorName == '!' && operand is TypeOperator && operand.operator == 'is') {
  ...
  void writeVariableDefinitions(VariableDeclarations vds) {
    if (vds.isConst) write('const '); else if (vds.isFinal) write('final ');
    if (vds.type != null) {
      writeType(vds.type);
      write(' ');
    }
  ...
  static StringLiteralOutput analyzeStringLiteral(Expression node) {
    ...
    void penalize(List<int> penalized, List<int> nonPenalized, int endIndex, num cost(tree.StringQuoting q)) {
      for (int j in penalized) {
        // Check if another track can benefit from switching from this track.
        for (int k in nonPenalized) {
          num newCost = best[j].cost // Whitespace in string juxtaposition
          + 1 // Whitespace in string juxtaposition
          + getQuoteCost(best[k].quoting);
          if (newCost < best[k].cost) {
            best[k] = new OpenStringChunk(best[j].end(endIndex), best[k].quoting, newCost);
          }
        }
    ...
    for (int i = 0; i < parts.length; i++) {
      ...
      if (part is int) {
        ...
        if (part is Identifier && !part.name.contains(r'$') && i + 1 < parts.length && isIdentifierPartNoDollar(parts[i + 1])) {
          for (int j in nonRaws) {
            for (int k = 0; k < best.length; k++) {
              ...
              + getQuoteCost(best[k].quoting);
              if (newCost < best[k].cost) {
                best[k] = new OpenStringChunk(best[j].end(i + 1), best[k].quoting, newCost);
              }
            }
          }
        }
  ...
  void writeStringLiteral(Expression node) {
    ...
    void printChunk(StringChunk chunk) {
      ...
      for (int i = startIndex; i < chunk.endIndex; i++) {
        ...
        } else if (part is Identifier && !part.name.contains(r'$') && (i == chunk.endIndex - 1 || !isIdentifierPartNoDollar(parts[i + 1]))) {
          write(r'$');
          write(part.name);
        } else {
          write(r'${');
