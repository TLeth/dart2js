...
import '../dart_types.dart' show DartType, FunctionType, InterfaceType, TypeKind;
import '../dart2jslib.dart' show ClassWorld, Compiler, Constant, FunctionConstant, invariant, TreeElementMapping;
import '../elements/elements.dart';
import '../native/native.dart' as native;
import '../tree/tree.dart' as ast show DartString, Node;
import '../types/types.dart' show ContainerTypeMask, DictionaryTypeMask, MapTypeMask, TypeMask, TypesInferrer, ValueTypeMask;
import '../universe/universe.dart' show Selector, SideEffects, TypedSelector;
import '../util/util.dart' show ImmutableEmptySet, Setlet, Spannable;
...
class TypeInformationSystem extends TypeSystem<TypeInformation> {
  ...
  final Map<Element, TypeInformation> typeInformations = new Map<Element, TypeInformation>();

  /// [ListTypeInformation] for allocated lists.
  final Map<ast.Node, TypeInformation> allocatedLists = new Map<ast.Node, TypeInformation>();

  /// [MapTypeInformation] for allocated Maps.
  final Map<ast.Node, TypeInformation> allocatedMaps = new Map<ast.Node, TypeInformation>();
  ...
  final Map<TypeMask, TypeInformation> concreteTypes = new Map<TypeMask, TypeInformation>();
  ...
  void withMember(MemberElement element, action) {
    assert(invariant(element, _currentMember == null, message: "Already constructing graph for $_currentMember."));
  ...
  TypeInformation get positiveIntType {
    ...
    return positiveIntTypeCache = getConcreteTypeFor(compiler.typesTask.positiveIntType);
  }
  ...
  TypeInformation get functionType {
    ...
    return functionTypeCache = getConcreteTypeFor(compiler.typesTask.functionType);
  }
  ...
  TypeInformation get constListType {
    ...
    return constListTypeCache = getConcreteTypeFor(compiler.typesTask.constListType);
  }

  TypeInformation fixedListTypeCache;
  TypeInformation get fixedListType {
    if (fixedListTypeCache != null) return fixedListTypeCache;
    return fixedListTypeCache = getConcreteTypeFor(compiler.typesTask.fixedListType);
  }
  ...
  TypeInformation get growableListType {
    ...
    return growableListTypeCache = getConcreteTypeFor(compiler.typesTask.growableListType);
  }
  ...
  TypeInformation get constMapType {
    ...
    return constMapTypeCache = getConcreteTypeFor(compiler.typesTask.constMapType);
  }
  ...
  TypeInformation get dynamicType {
    ...
    return dynamicTypeCache = getConcreteTypeFor(compiler.typesTask.dynamicType);
  }

  TypeInformation nonNullEmptyType;

  TypeInformation stringLiteralType(ast.DartString value) {
    return new StringLiteralTypeInformation(value, compiler.typesTask.stringType);
  }

  TypeInformation computeLUB(TypeInformation firstType, TypeInformation secondType) {
    ...
    return getConcreteTypeFor(firstType.type.union(secondType.type, classWorld));
  }
  ...
  TypeInformation narrowType(TypeInformation type, DartType annotation, {bool isNullable: true}) {
    ...
    if (annotation.isTypedef || annotation.isFunctionType) {
      ...
      otherType = annotation.element == classWorld.objectClass ? dynamicType.type.nonNullable() : new TypeMask.nonNullSubtype(annotation.element, classWorld);
    }
  ...
  TypeInformation nonNullSubtype(ClassElement type) {
    return getConcreteTypeFor(new TypeMask.nonNullSubtype(type.declaration, classWorld));
  }

  TypeInformation nonNullSubclass(ClassElement type) {
    return getConcreteTypeFor(new TypeMask.nonNullSubclass(type.declaration, classWorld));
  }

  TypeInformation nonNullExact(ClassElement type) {
    return getConcreteTypeFor(new TypeMask.nonNullExact(type.declaration, classWorld));
  }
  ...
  TypeInformation allocateList(TypeInformation type, ast.Node node, Element enclosing, [TypeInformation elementType, int length]) {
    bool isTypedArray = (compiler.typedDataClass != null) && type.type.satisfies(compiler.typedDataClass, classWorld);
    bool isConst = (type.type == compiler.typesTask.constListType);
    bool isFixed = (type.type == compiler.typesTask.fixedListType) || isConst || isTypedArray;
    bool isElementInferred = isConst || isTypedArray;

    int inferredLength = isFixed ? length : null;
    TypeMask elementTypeMask = isElementInferred ? elementType.type : dynamicType.type;
    ContainerTypeMask mask = new ContainerTypeMask(type.type, node, enclosing, elementTypeMask, inferredLength);
    ElementInContainerTypeInformation element = new ElementInContainerTypeInformation(currentMember, elementType);
    element.inferred = isElementInferred;

    allocatedTypes.add(element);
    return allocatedLists[node] = new ListTypeInformation(currentMember, mask, element, length);
  }

  TypeInformation allocateClosure(ast.Node node, Element element) {
    TypeInformation result = new ClosureTypeInformation(currentMember, node, element);
    allocatedClosures.add(result);
    return result;
  }

  TypeInformation allocateMap(ConcreteTypeInformation type, ast.Node node, Element element, [List<TypeInformation> keyTypes, List<TypeInformation> valueTypes]) {
    ...
    TypeMask valueType;
    if (isFixed) {
      keyType = keyTypes.fold(nonNullEmptyType.type, (type, info) => type.union(info.type, classWorld));
      valueType = valueTypes.fold(nonNullEmptyType.type, (type, info) => type.union(info.type, classWorld));
    } else {
      keyType = valueType = dynamicType.type;
    }
    MapTypeMask mask = new MapTypeMask(type.type, node, element, keyType, valueType);

    TypeInformation keyTypeInfo = new KeyInMapTypeInformation(currentMember, null);
    TypeInformation valueTypeInfo = new ValueInMapTypeInformation(currentMember, null);
    allocatedTypes.add(keyTypeInfo);
    allocatedTypes.add(valueTypeInfo);

    MapTypeInformation map = new MapTypeInformation(currentMember, mask, keyTypeInfo, valueTypeInfo);

    for (int i = 0; i < keyTypes.length; ++i) {
      TypeInformation newType = map.addEntryAssignment(keyTypes[i], valueTypes[i], true);
  ...
  Selector newTypedSelector(TypeInformation info, Selector selector) {
    ...
    // analysis.
    return info.isConcrete ? new TypedSelector(info.type, selector, classWorld) : selector;
  }

  TypeInformation allocateDiamondPhi(TypeInformation firstInput, TypeInformation secondInput) {
    PhiElementTypeInformation result = new PhiElementTypeInformation(currentMember, null, false, null);
  ...
  PhiElementTypeInformation allocatePhi(ast.Node node, Local variable, inputType) {
    // Check if [inputType] is a phi for a local updated in
    // the try/catch block [node]. If it is, no need to allocate a new
    // phi.
    if (inputType is PhiElementTypeInformation && inputType.branchNode == node) {
      return inputType;
    }
    PhiElementTypeInformation result = new PhiElementTypeInformation(currentMember, node, true, variable);
    allocatedTypes.add(result);
    result.addAssignment(inputType);
    return result;
  }

  TypeInformation simplifyPhi(ast.Node node, Local variable, PhiElementTypeInformation phiType) {
    if (phiType.assignments.length == 1) return phiType.assignments.first;
    return phiType;
  }

  PhiElementTypeInformation addPhiInput(Local variable, PhiElementTypeInformation phiType, TypeInformation newType) {
 ...
 *
 */
class TypeGraphInferrerEngine extends InferrerEngine<TypeInformation, TypeInformationSystem> {
  final Map<Element, TypeInformation> defaultTypeOfParameter = new Map<Element, TypeInformation>();
  final List<CallSiteTypeInformation> allocatedCalls = <CallSiteTypeInformation>[];
   ...
   * their element type.
   */
  final Set<Selector> _returnsListElementTypeSet = new Set<Selector>.from(<Selector>[new Selector.getter('first', null), new Selector.getter('last', null), new Selector.getter('single', null), new Selector.call('singleWhere', null, 1), new Selector.call('elementAt', null, 1), new Selector.index(), new Selector.call('removeAt', null, 1), new Selector.call('removeLast', null, 0)]);

  bool returnsListElementType(Selector selector) {
    return (selector.mask != null) && selector.mask.isContainer && _returnsListElementTypeSet.contains(selector.asUntyped);
  }

  bool returnsMapValueType(Selector selector) {
    return (selector.mask != null) && selector.mask.isMap && selector.isIndex;
  }
  ...
  void analyzeMapAndEnqueue(MapTypeInformation info) {
    ...
    for (int i = 0; i < tracer.keyAssignments.length; ++i) {
      TypeInformation newType = info.addEntryAssignment(tracer.keyAssignments[i], tracer.valueAssignments[i]);
  ...
  void runOverAllElements() {
    ...
    types.allocatedClosures.forEach((info) {
      void trace(Iterable<FunctionElement> elements, ClosureTracerVisitor tracer) {
        tracer.run();
        if (!tracer.continueAnalyzing) {
          elements.forEach((FunctionElement e) {
            ...
            e.functionSignature.forEachParameter((parameter) {
              types.getInferredTypeOf(parameter).giveUp(this, clearAssignments: false);
            });
          });
          bailedOutOn.addAll(elements);
          return;
        }
        elements.where((e) => !bailedOutOn.contains(e)).forEach((FunctionElement e) {
          e.functionSignature.forEachParameter((parameter) {
          ...
          ;
          if (_VERBOSE) {
            print("traced closure $e as " "${compiler.world.getMightBePassedToApply(e)}");
          }
        });
      }
      if (info is ClosureTypeInformation) {
        ...
        Iterable<FunctionElement> elements = info.callees.where((e) => e.isFunction);
        trace(elements, new ClosureTracerVisitor(elements, info, this));
      } else {
        assert(info is ElementTypeInformation);
        trace([info.element], new StaticTearOffClosureTracerVisitor(info.element, info, this));
      }
    });
    ...
    if (_PRINT_SUMMARY) {
      types.allocatedLists.values.forEach((ListTypeInformation info) {
        print('${info.type} ' 'for ${info.originalType.allocationNode} ' 'at ${info.originalType.allocationElement} ' 'after ${info.refineCount}');
      });
      types.allocatedMaps.values.forEach((MapTypeInformation info) {
        print('${info.type} ' 'for ${info.originalType.allocationNode} ' 'at ${info.originalType.allocationElement} ' 'after ${info.refineCount}');
      });
      types.allocatedClosures.forEach((TypeInformation info) {
        if (info is ElementTypeInformation) {
          print('${types.getInferredSignatureOf(info.element)} for ' '${info.element}');
  ...
  void analyze(Element element, ArgumentsTypes arguments) {
    ...
    SimpleTypeInferrerVisitor visitor = new SimpleTypeInferrerVisitor(element, compiler, this);
    ...
    if (element.isField) {
      ...
      if (element.isFinal || element.isConst) {
        ...
        if (fieldElement.initializer != null) {
          if (type is! ListTypeInformation && type is! MapTypeInformation) {
            ...
            ConstantExpression constant = compiler.backend.constants.getConstantForVariable(element);
            if (constant != null) {
      ...
      if (Elements.isStaticOrTopLevelField(element) && fieldElement.initializer != null && !element.isConst) {
        var argument = fieldElement.initializer;
        // TODO(13429): We could do better here by using the
        // constant handler to figure out if it's a lazy field or not.
        if (argument.asSend() != null || (argument.asNewExpression() != null && !argument.isConst)) {
          recordType(element, types.nullType);
        }
      }
    } else {
  ...
  void processLoopInformation() {
    allocatedCalls.forEach((info) {
      ...
      } else if (info.selector.mask != null && !info.selector.mask.containsAll(compiler.world)) {
   ...
   */
  void updateParameterAssignments(TypeInformation caller, Element callee, ArgumentsTypes arguments, Selector selector, {bool remove, bool addToQueue: true}) {
    ...
    if (callee.isField) {
      ...
      signature.forEachParameter((Element parameter) {
        if (signature.hasOptionalParameters && parameter == signature.firstOptionalParameter) {
          visitingRequiredParameter = false;
        }
        TypeInformation type = visitingRequiredParameter ? arguments.positional[parameterIndex] : signature.optionalParametersAreNamed ? arguments.named[parameter.name] : parameterIndex < arguments.positional.length ? arguments.positional[parameterIndex] : null;
   ...
   * updated.
   */
  void setDefaultTypeOfParameter(ParameterElement parameter, TypeInformation type) {
  ...
  void recordTypeOfFinalField(Spannable node, Element analyzed, Element element, TypeInformation type) {
    types.getInferredTypeOf(element).addAssignment(type);
  }

  void recordTypeOfNonFinalField(Spannable node, Element element, TypeInformation type) {
    types.getInferredTypeOf(element).addAssignment(type);
  }
  ...
  TypeInformation addReturnTypeFor(Element element, TypeInformation unused, TypeInformation newType) {
  ...
  TypeInformation registerCalledElement(Spannable node, Selector selector, Element caller, Element callee, ArgumentsTypes arguments, SideEffects sideEffects, bool inLoop) {
    CallSiteTypeInformation info = new StaticCallSiteTypeInformation(types.currentMember, node, caller, callee, selector, arguments, inLoop);
  ...
  TypeInformation registerCalledSelector(ast.Node node, Selector selector, TypeInformation receiverType, Element caller, ArgumentsTypes arguments, SideEffects sideEffects, bool inLoop) {
    if (selector.isClosureCall) {
      return registerCalledClosure(node, selector, receiverType, caller, arguments, sideEffects, inLoop);
    }
    ...
    CallSiteTypeInformation info = new DynamicCallSiteTypeInformation(types.currentMember, node, caller, selector, receiverType, arguments, inLoop);
  ...
  TypeInformation registerCalledClosure(ast.Node node, Selector selector, TypeInformation closure, Element caller, ArgumentsTypes arguments, SideEffects sideEffects, bool inLoop) {
    sideEffects.setDependsOnSomething();
    sideEffects.setAllSideEffects();
    CallSiteTypeInformation info = new ClosureCallSiteTypeInformation(types.currentMember, node, caller, selector, closure, arguments, inLoop);
    info.addToGraph(this);
    allocatedCalls.add(info);
    return info;
  }
  ...
  Iterable<Element> sortResolvedElements() {
    ...
    compiler.enqueuer.resolution.resolvedElements.forEach((AstElement element) {
      ...
      assert(invariant(element, element.isField || element.isFunction || element.isGenerativeConstructor || element.isGetter || element.isSetter, message: 'Unexpected element kind: ${element.kind}'));
      if (element.isAbstract) return;
      // Put the other operators in buckets by length, later to be added in
      // length order.
      int length = mapping.getSelectorCount();
      max = length > max ? length : max;
      Setlet<Element> set = methodSizes.putIfAbsent(length, () => new Setlet<Element>());
      set.add(element);
    });
  ...
  Iterable<Element> getCallersOf(Element element) {
    if (compiler.disableTypeInference) {
      throw new UnsupportedError("Cannot query the type inferrer when type inference is disabled.");
    }
   ...
   */
  TypeInformation typeOfElementWithSelector(Element element, Selector selector) {
    if (element.name == Compiler.NO_SUCH_METHOD && selector.name != element.name) {
      ...
      if (element.isFunction) {
        ...
        return types.functionType == null ? types.dynamicType : types.functionType;
...
class TypeGraphInferrer implements TypesInferrer {
  ...
  TypeMask getTypeOfSelector(Selector selector) {
    ...
    for (Element element in elements) {
      TypeMask type = inferrer.typeOfElementWithSelector(element, selector).type;
