...
import '../../../js_lib/shared/embedded_names.dart' show DEFERRED_LIBRARY_URIS, DEFERRED_LIBRARY_HASHES, INITIALIZE_LOADED_HUNK, IS_HUNK_LOADED;
...
class ModelEmitter {
  ...
  static const String deferredInitializersGlobal = r"$__dart_deferred_initializers__";
  ...
  js.Statement emitMainUnit(MainOutput unit, Map<String, List<Output>> loadMap) {
    js.Expression program = new js.ArrayInitializer.from(unit.libraries.map(emitLibrary));
    return js.js.statement(boilerplate, [emitDeferredInitializerGlobal(loadMap), emitHolders(unit.holders), emitEmbeddedGlobals(loadMap), emitConstants(unit.constants), emitStaticNonFinalFields(unit.staticNonFinalFields), unit.main, program]);
  }
  ...
  js.Block emitHolders(List<Holder> holders) {
    ...
    List<js.Statement> statements = [new js.ExpressionStatement(new js.VariableDeclarationList(holders.map((e) => new js.VariableInitialization(new js.VariableDeclaration(e.name, allowRename: false), new js.ObjectInitializer(const []))).toList())), js.js.statement('var holders = #', new js.ArrayInitializer.from(holders.map((e) => new js.VariableUse(e.name))))];
  ...
  js.Block emitEmbeddedGlobals(Map<String, List<Output>> loadMap) {
    ...
    List<js.Statement> statements = [new js.ExpressionStatement(new js.VariableDeclarationList([new js.VariableInitialization(new js.VariableDeclaration(r"init", allowRename: false), globalsObject)]))];
    return new js.Block(statements);
  }

  List<js.Property> emitLoadUrisAndHashes(Map<String, List<Output>> loadMap) {
    js.ArrayInitializer outputUris(List<Output> outputs) {
      return js.stringArray(outputs.map((DeferredOutput output) => "${output.outputFileName}$deferredExtension"));
    }
    js.ArrayInitializer outputHashes(List<Output> outputs) {
      // TODO(floitsch): the hash must depend on the generated code.
      return js.numArray(outputs.map((DeferredOutput output) => output.hashCode));
    }
    ...
    loadMap.forEach((String loadId, List<Output> outputList) {
      ...
      hashes[count] = new js.Property(js.string(loadId), outputHashes(outputList));
      count++;
    });

    return <js.Property>[new js.Property(js.string(DEFERRED_LIBRARY_URIS), new js.ObjectInitializer(uris)), new js.Property(js.string(DEFERRED_LIBRARY_HASHES), new js.ObjectInitializer(hashes))];
  }
  ...
  js.Property emitIsHunkLoadedFunction() {
    js.Expression function = js.js("function(hash) { return !!$deferredInitializersGlobal[hash]; }");
    return new js.Property(js.string(IS_HUNK_LOADED), function);
  }

  js.Property emitInitializeLoadedHunk() {
    js.Expression function = js.js("function(hash) { eval($deferredInitializersGlobal[hash]); }");
  ...
  js.Expression emitDeferredUnit(DeferredOutput unit, List<Holder> holders) {
    ...
    js.ArrayInitializer content = new js.ArrayInitializer.from(unit.libraries.map(emitLibrary));
  ...
  js.Block emitConstants(List<Constant> constants) {
    Iterable<js.Statement> statements = constants.map((Constant constant) {
      js.Expression code = constantEmitter.initializationExpression(constant.value);
      return js.js.statement("#.# = #;", [constant.holder.name, constant.name, code]);
    });
  ...
  js.Block emitStaticNonFinalFields(List<StaticField> fields) {
    Iterable<js.Statement> statements = fields.map((StaticField field) {
      return js.js.statement("#.# = #;", [field.holder.name, field.name, field.code]);
    });
    return new js.Block(statements.toList());
  }

  js.Expression emitLibrary(Library library) {
    Iterable staticDescriptors = library.statics.expand((e) => [js.string(e.name), js.number(e.holder.index), emitStaticMethod(e)]);
    Iterable classDescriptors = library.classes.expand((e) => [js.string(e.name), js.number(e.holder.index), emitClass(e)]);
  ...
  js.Expression emitClass(Class cls) {
    List elements = [js.string(cls.superclassName), js.number(cls.superclassHolderIndex)];
